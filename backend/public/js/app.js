...
// ---------------- VIEW SWITCHING ----------------
const ALL_VIEWS = [
  'role-select-view', 'employee-login-view', 'manager-login-view', 'krystal-login-view', 'admin-login-view',
...
// ---------------- URL ROUTING ----------------
// Supports direct navigation: /employee, /manager, /krystal, /admin, and / (role select)
function pathToRole(path) {
  const clean = path.replace(/\/+$/, '') || '/';
  if (clean === '/employee') return 'employee';
  if (clean === '/manager') return 'manager';
  if (clean === '/krystal') return 'krystal';
...
function renderRoute() {
  ...
  const staffRoles = ['admin', 'manager', 'krystal'];
...
document.getElementById('krystal-login-btn').addEventListener('click', () => doStaffLogin('krystal'));
document.getElementById('krystal-login-password').addEventListener('keydown', e => { if (e.key === 'Enter') doStaffLogin('krystal'); });
...
function showDashboard() {
  const role = getRole();
  document.body.classList.remove('role-admin', 'role-manager', 'role-krystal');
  ...
  // If a non-admin (manager/krystal) landed on an admin-only tab from a previous admin
...
document.querySelectorAll('.nav-item[data-tab]').forEach(item => {
  item.addEventListener('click', () => {
    ...
    if (item.dataset.tab === 'reports') prefillReportDatesIfEmpty();
    if (item.dataset.tab === 'employees') loadEmployees();
    if (item.dataset.tab === 'managers') { loadManagers(); loadKrystalAccounts(); }
  });
});
...
document.getElementById('download-summary-excel-btn').addEventListener('click', () => {
  ...
  fetch(url, { headers: { Authorization: `Bearer ${getToken()}` } })
    ...
    .then(blob => {
      ...
      const objUrl = window.URL.createObjectURL(blob);
      a.href = objUrl;
      a.download = `MTDC_PA_Report.xlsx`;
      document.body.appendChild(a);
      a.click();
      a.remove();
...
document.getElementById('download-employees-excel-btn').addEventListener('click', () => {
  ...
  const url = `${API}/export/employees-excel${location ? '?location=' + encodeURIComponent(location) : ''}`;
  ...
  fetch(url, { headers: { Authorization: `Bearer ${getToken()}` } })
    ...
    .then(blob => {
      ...
      const objUrl = window.URL.createObjectURL(blob);
      a.href = objUrl;
      a.download = `MTDC_Employee_Data.xlsx`;
      document.body.appendChild(a);
      a.click();
      a.remove();
...
async function loadEmployees() {
  try {
    ...
    if (data.employees.length === 0) {
      tbody.innerHTML = `<tr><td colspan="8"><div class="empty-state"><div class="icon">👥</div>No employees added yet</div></td></tr>`;
      return;
    }
...
document.getElementById('add-emp-btn').addEventListener('click', async () => {
  ...
  try {
    await apiFetch('/employees', {
      method: 'POST',
      body: JSON.stringify({ employee_id, name, designation, phone, location, doj })
    });
...
async function submitBulkEmployees(employees) {
  const valid = employees.filter(e => e.employee_id && e.name);
  const skipped = employees.length - valid.length;

  if (valid.length === 0) {
    showToast('No valid rows found (Employee ID and Name are required)', true);
    return;
  }
  ...
  try {
    ...
    showToast(`${data.message}${skipped ? ` · ${skipped} row(s) skipped (missing ID/Name)` : ''}`);
    loadEmployees();
  } catch (err) {
    showToast(err.message, true);
  }
...
document.getElementById('bulk-emp-paste-btn').addEventListener('click', () => {
  ...
  const employees = lines.map(line => {
    ...
    const [employee_id, name, designation, phone, location, doj] = parts;
    return { employee_id, name, designation, phone, location, doj };
  });
...
// ---------------- KRYSTAL ACCOUNTS (admin only, same access level as Manager) ----------------
async function loadKrystalAccounts() {
  if (getRole() !== 'admin') return;
  try {
    const data = await apiFetch('/auth/krystal-accounts');
    document.getElementById('krystal-count').textContent = `Krystal Account List (${data.count})`;
    const tbody = document.getElementById('krystal-table-body');
    if (data.accounts.length === 0) {
      tbody.innerHTML = `<tr><td colspan="3"><div class="empty-state"><div class="icon">💎</div>No Krystal accounts yet</div></td></tr>`;
      return;
    }
    tbody.innerHTML = data.accounts.map(a => `
      <tr>
        <td class="mono">${a.username}</td>
        <td class="mono">${new Date(a.created_at).toLocaleDateString('en-IN')}</td>
        <td><button class="btn secondary small" onclick="deleteKrystalAccount(${a.id})">Remove</button></td>
      </tr>
    `).join('');
  } catch (err) {
    showToast(err.message, true);
...
document.getElementById('add-krystal-btn').addEventListener('click', async () => {
  const username = document.getElementById('add-krystal-username').value.trim();
  const password = document.getElementById('add-krystal-password').value;
  if (!username || !password || password.length < 6) {
    showToast('Username and password (min 6 characters) are required', true);
    return;
  }
  try {
    await apiFetch('/auth/krystal-accounts', {
      method: 'POST',
      body: JSON.stringify({ username, password })
    });
    showToast(`Krystal account ${username} added successfully`);
    document.getElementById('add-krystal-username').value = '';
    document.getElementById('add-krystal-password').value = '';
    loadKrystalAccounts();
  } catch (err) {
    showToast(err.message, true);
  }
...
async function deleteKrystalAccount(id) {
  try {
    await apiFetch(`/auth/krystal-accounts/${id}`, { method: 'DELETE' });
    showToast('Krystal account removed');
    loadKrystalAccounts();
  } catch (err) {
    showToast(err.message, true);
  }
