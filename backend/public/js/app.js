...
function saveSession(token, role, employee, project, roleLabel, companyName, settings) {
...
// Hides nav items for Functions the platform owner has switched OFF for this company. Safe
// to call every time a dashboard is shown (login, or session restore on page reload) — if
// settings aren't known yet (e.g. old cached session before this feature existed), every
// nav item is simply left visible, since the backend still enforces the real restriction.
...
const ALL_VIEWS = [
  ...
  'employee-dashboard-view', 'dashboard-view', 'companies-view', 'logged-out-view'
];
...
async function doEmployeeLogin() {
  ...
  try {
    ...
    saveSession(data.token, 'employee', data.employee, null, null, data.company && data.company.name, data.settings);
...
async function doStaffLogin(kind) {
  ...
  try {
    ...
    rememberCompanyCode(company_code);
    saveSession(data.token, data.role, null, data.project || null, data.custom_role_name || null, data.company_name || null, data.settings);
...
document.getElementById('add-emp-btn').addEventListener('click', async () => {
  ...
  if (!pin || !/^\d{4,6}$/.test(pin)) {
    showToast('PIN is required and must be 4-6 digits (used for employee login)', true);
    return;
  }
...
document.getElementById('edit-emp-save-btn').addEventListener('click', async () => {
  ...
  try {
    await apiFetch(`/employees/${employeeId}`, {
      method: 'PUT',
      body: JSON.stringify({ name, pin, designation, phone, location, doj, project, shift_category, zone, ward, site_code })
    });
...
function openCompanySettingsModal(id) {
  ...
  document.getElementById('company-modal-error').style.display = 'none';
  // Name/Code/Admin fields aren't editable from this shortcut — hide them, only show the
  // Functions + Report Columns checkboxes.
...
document.getElementById('company-modal-save').addEventListener('click', async () => {
  ...
  if (companyModalMode === 'settings') {
    try {
      await apiFetch(`/companies/${companyModalEditingId}/settings`, {
        method: 'PUT',
        body: JSON.stringify({ features, report_columns })
      });
  ...
  try {
    await apiFetch('/companies', {
      method: 'POST',
      body: JSON.stringify({ name, code, admin_username, admin_password, features, report_columns })
    });
