...
function saveSession(token, role, employee, project, roleLabel, companyName) {
...
async function doEmployeeLogin() {
  ...
  try {
    ...
    saveSession(data.token, 'employee', data.employee, null, null, data.company && data.company.name);
...
async function doStaffLogin(kind) {
  ...
  try {
    ...
    rememberCompanyCode(company_code);
    saveSession(data.token, data.role, null, data.project || null, data.custom_role_name || null, data.company_name || null);
...
document.getElementById('company-modal-save').addEventListener('click', async () => {
  ...
  try {
    await apiFetch('/companies', {
      method: 'POST',
      body: JSON.stringify({ name, code, admin_username, admin_password })
    });
