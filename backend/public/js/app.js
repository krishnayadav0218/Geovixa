...
async function loadSiteLocationsList(project) {
  try {
    ...
    if (!data.locations.length) {
      tbody.innerHTML = `<tr><td colspan="6"><div class="empty-state"><div class="icon">📍</div>No locations yet — add one below</div></td></tr>`;
      return;
    }
...
document.getElementById('site-loc-add-btn').addEventListener('click', async () => {
  ...
  try {
    await apiFetch('/site-locations', { method: 'POST', body: JSON.stringify({ project, name, latitude, longitude, radius_m }) });
...
document.getElementById('site-loc-bulk-btn').addEventListener('click', async () => {
  ...
  const locations = text.split('\n').map(line => {
    ...
    return { name: parts[0], latitude: parts[1], longitude: parts[2], radius_m: parts[3] || 200 };
  }).filter(l => l.name);
