...
// ---------------- PROJECTS ----------------
// Populates every Project <select> in the app (Add Employee, filters, reports) and,
// for admins, renders the removable chip list in "Manage Projects".
async function loadProjects() {
  try {
    ...
    const selectIds = ['add-emp-project', 'emp-filter-project', 'filter-project', 'report-project', 'employee-report-project'];
    selectIds.forEach(id => {
      const sel = document.getElementById(id);
      if (!sel) return;
      const prevValue = sel.value;
      const placeholder = sel.querySelector('option')?.outerHTML || '<option value="">— Select Project —</option>';
      sel.innerHTML = placeholder + projects.map(p => `<option value="${p.name}">${p.name}</option>`).join('');
      if (projects.some(p => p.name === prevValue)) sel.value = prevValue;
    });
    ...
    if (listBox) {
      if (projects.length === 0) {
        ...
        listBox.innerHTML = projects.map(p => `
