...
document.getElementById('employee-login-pin').addEventListener('keydown', e => { if (e.key === 'Enter') doEmployeeLogin(); });
...
async function doEmployeeLogin() {
  ...
  if (!pin) {
    errBox.textContent = 'Please enter your PIN';
    errBox.style.display = 'block';
    return;
  }
...
document.getElementById('download-punch-excel-btn').addEventListener('click', () => {
  ...
  fetch(url, { headers: { Authorization: `Bearer ${getToken()}` } })
    ...
    .then(blob => {
      ...
      const objUrl = window.URL.createObjectURL(blob);
      a.href = objUrl;
      a.download = `Krystal_Connect_Punch_Time_Report.xlsx`;
      document.body.appendChild(a);
      a.click();
      a.remove();
...
async function loadProjects() {
  try {
    ...
    const selectIds = ['add-emp-project', 'edit-emp-project', 'filter-project', 'report-project', 'punch-report-project', 'employee-report-project', 'add-mgr-project', 'add-coordinator-project', 'edit-mgr-project'];
...
document.getElementById('add-emp-btn').addEventListener('click', async () => {
  ...
  if (!/^\d{4,6}$/.test(pin)) {
    showToast('Login PIN must be 4 to 6 digits', true);
    return;
  }
  ...
  try {
    await apiFetch('/employees', {
      method: 'POST',
      body: JSON.stringify({
        employee_id, name, pin, designation, phone, location, doj, project, shift_category,
        zone, ward, site_code,
...
function openEditEmployeeModal(employeeId) {
  ...
  document.getElementById('edit-emp-pin').value = ''; // never prefilled — blank means "keep current PIN"
...
document.getElementById('edit-emp-save-btn').addEventListener('click', async () => {
  ...
  try {
    const body = { name, designation, phone, location, doj, project, shift_category, zone, ward, site_code };
    if (pin) body.pin = pin;
    await apiFetch(`/employees/${employeeId}`, {
      method: 'PUT',
      body: JSON.stringify(body)
    });
    showToast(`Employee ${employeeId} updated successfully${pin ? ' — PIN reset' : ''}`);
...
const BULK_HEADER_MAP = {
  ...
  esic: ['esic', 'e.s.i.c.'],
  pin: ['pin', 'login pin', 'employee pin'],
};
...
async function submitBulkEmployees(employees) {
  ...
  try {
    ...
    if (data.generatedPins && data.generatedPins.length) {
      downloadGeneratedPins(data.generatedPins);
      showToast(`${data.generatedPins.length} login PIN(s) generated — a file just downloaded, distribute securely to each employee`);
    }
    loadEmployees();
  } catch (err) {
    showToast(err.message, true);
...
// Triggers a browser download of the auto-generated employee PINs from a bulk import,
// so the admin has something concrete to hand out instead of digging through dev tools.
// Nothing here is stored anywhere after this — the PINs only ever exist as bcrypt hashes
// in the database from this point on.
function downloadGeneratedPins(generatedPins) {
  const lines = ['Employee ID,PIN', ...generatedPins.map(p => `${p.employee_id},${p.pin}`)];
  const blob = new Blob([lines.join('\n')], { type: 'text/csv' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `krystal-connect-generated-pins-${Date.now()}.csv`;
  document.body.appendChild(a);
  a.click();
  a.remove();
