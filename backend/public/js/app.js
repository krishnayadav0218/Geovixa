...
document.querySelectorAll('.nav-item[data-tab]').forEach(item => {
  item.addEventListener('click', () => {
    ...
    if (item.dataset.tab === 'reports') loadReportGrid();
...
// ---------------- REPORTS (P/A grid, admin + manager) ----------------
...
async function loadReportGrid() {
  ...
  try {
    const data = await apiFetch(`/attendance/grid?from=${fromInput.value}&to=${toInput.value}`);

    document.getElementById('report-grid-count').textContent = `Report Preview (${data.employees.length} employees × ${data.dates.length} days)`;

    const thead = document.getElementById('report-grid-head');
    thead.innerHTML = `<tr>
      <th>Employee ID</th><th>Name</th><th>Location</th><th>DOJ</th>
      ${data.dates.map(d => `<th class="mono">${d.slice(5)}</th>`).join('')}
    </tr>`;

    const tbody = document.getElementById('report-grid-body');
    if (data.employees.length === 0) {
      tbody.innerHTML = `<tr><td colspan="${4 + data.dates.length}"><div class="empty-state"><div class="icon">📭</div>No employees to show</div></td></tr>`;
      return;
    }
    ...
    tbody.innerHTML = data.employees.map(e => `
      <tr>
        ...
        ${data.dates.map(d => `<td class="mono" style="text-align:center;font-weight:700;color:${e.days[d] === 'P' ? '#2E7D32' : e.days[d] === 'A' ? '#C62828' : 'var(--text-muted)'}">${e.days[d]}</td>`).join('')}
      </tr>
    `).join('');
  } catch (err) {
    showToast(err.message, true);
...
document.getElementById('apply-report-btn').addEventListener('click', loadReportGrid);

document.getElementById('download-summary-excel-btn').addEventListener('click', () => {
  const from = document.getElementById('report-from').value;
  const to = document.getElementById('report-to').value;
  const d = (!from || !to) ? defaultReportDates() : { from, to };

  const url = `${API}/export/summary-excel?from=${d.from}&to=${d.to}`;
