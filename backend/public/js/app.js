...
async function loadMyHistory() {
  try {
    ...
    document.getElementById('my-history-count').textContent = `My Attendance History (${data.count})`;
    const tbody = document.getElementById('my-history-body');
    if (data.records.length === 0) {
      tbody.innerHTML = `<tr><td colspan="5"><div class="empty-state"><div class="icon">📭</div>No attendance marked yet</div></td></tr>`;
      return;
    }
    const emp = getEmployeeInfo();
    tbody.innerHTML = data.records.map(r => {
      const meta = {
        ...
        name: emp ? emp.name : '',
        address: r.address || (r.latitude.toFixed(5) + ', ' + r.longitude.toFixed(5)),
        date: r.attendance_date,
        time: new Date(r.device_time || r.server_time).toLocaleTimeString('en-IN', { timeZone: 'Asia/Kolkata', hour: '2-digit', minute: '2-digit', hour12: true }),
      };
      return `
      <tr>
        <td>${photoThumb(r.photo, meta)}</td>
...
async function openCameraModal(status) {
  ...
  navigator.geolocation.getCurrentPosition(
    ...
    (err) => {
      document.getElementById('camera-status').textContent = 'Location access denied. Please allow location permission.';
    },
    ...
    if (summaryData.summary.length === 0) {
      ...
      tbody.innerHTML = summaryData.summary.map(s => {
        const meta = {
          ...
          time: s.time ? new Date(s.time).toLocaleTimeString('en-IN', { timeZone: 'Asia/Kolkata', hour: '2-digit', minute: '2-digit', hour12: true }) : '',
        };
        return `
        <tr>
          <td>${photoThumb(s.photo, meta)}</td>
...
// meta = { employee_id, name, address, date, time }
function photoThumb(photo, meta) {
  if (!photo) return `<div class="photo-thumb photo-thumb-empty">—</div>`;
  const encodedMeta = encodeURIComponent(JSON.stringify(meta || {}));
  return `<img src="${photo}" class="photo-thumb" data-meta="${encodedMeta}" onclick="showPhotoModal(this)" />`;
}

function showPhotoModal(imgEl) {
  const modal = document.getElementById('photo-modal');
  document.getElementById('photo-modal-img').src = imgEl.src;

  let meta = {};
  try { meta = JSON.parse(decodeURIComponent(imgEl.dataset.meta || '%7B%7D')); } catch (e) { meta = {}; }

  const overlay = document.getElementById('photo-modal-overlay');
  overlay.innerHTML = `
    <div class="pm-id">ID: ${meta.employee_id || '-'}</div>
    <div class="pm-name">${meta.name || ''}</div>
    <div>${meta.address || ''}</div>
    <div>${meta.date || ''} ${meta.time || ''}</div>
  `;
