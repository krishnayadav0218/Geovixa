...
const ALL_VIEWS = [
  ...
  'super-admin-login-view', 'client-login-view',
...
function activateNavTab(tabKey) {
  ...
  if (tabKey === 'reliever') { loadRelieverDashboard(); startRelieverAutoRefresh(); }
  if (tabKey === 'overtime') { loadOvertimeRequests(); loadPaymentBatches(); }
  if (tabKey === 'ops-map') { loadOpsMap(); startOpsMapAutoRefresh(); }
  ...
  try {
    ...
    tbody.innerHTML = data.clients.length ? data.clients.map(c => `
      <tr>
        ...
        <td><button class="btn small secondary" onclick="deleteClientAccount(${c.id})">Delete</button></td>
      </tr>
...
document.getElementById('client-new-cancel-btn').addEventListener('click', () => document.getElementById('client-new-modal').classList.add('hidden'));
...
document.getElementById('reliever-rank-submit-btn').addEventListener('click', async () => {
  ...
  const doRank = async (lat, lng) => {
    try {
      ...
      results.innerHTML = data.ranked.map(r => `
        <div class="panel" style="margin-bottom:8px;padding:10px 14px;">
          <div style="display:flex;justify-content:space-between;align-items:center;">
            ...
            <div style="text-align:right;">
              <div style="font-weight:800;font-size:16px;">${r.score}</div>
