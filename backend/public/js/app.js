...
document.querySelectorAll('.nav-item[data-tab]').forEach(item => {
  item.addEventListener('click', () => {
    ...
    if (item.dataset.tab === 'managers') { loadManagers(); loadCoordinatorAccounts(); }
  });
});
...
async function loadCoordinatorAccounts() {
  ...
  try {
    ...
    if (data.accounts.length === 0) {
      tbody.innerHTML = `<tr><td colspan="3"><div class="empty-state"><div class="icon">💎</div>No Coordinator accounts yet</div></td></tr>`;
      return;
    }
