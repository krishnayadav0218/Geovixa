...
function activateNavTab(tabKey) {
  ...
  if (tabKey === 'reports') { loadProjects(); prefillReportDatesIfEmpty(); }
  if (tabKey === 'employees') { loadProjects(); loadShiftCategories(); showEmployeeProjectPicker(); }
  if (tabKey === 'salary-requests') loadSalaryRequests();
  if (tabKey === 'managers') { loadProjects(); loadManagers(); loadCoordinatorAccounts(); loadRoleTypes(); loadRoleAccounts(); }
  return true;
}
...
async function loadProjects() {
  try {
    ...
    const selectIds = ['add-emp-project', 'edit-emp-project', 'filter-project', 'report-project', 'punch-report-project', 'employee-report-project', 'add-mgr-project', 'add-coordinator-project', 'edit-mgr-project', 'add-role-project', 'edit-role-account-project'];
