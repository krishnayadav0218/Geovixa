...
function saveSession(token, role, employee, project, roleLabel, companyName, settings, companyLogoUrl) {
  ...
  // Employees are the only role whose live location gets tracked. Ask for background
  // location permission right after they log in (inside the Android app only — this is a
  // no-op in a normal browser since window.Capacitor won't exist there).
  if (role === 'employee' && window.Capacitor && window.Capacitor.isNativePlatform && window.Capacitor.isNativePlatform() && window.Capacitor.Plugins && window.Capacitor.Plugins.BackgroundTracker) {
    window.Capacitor.Plugins.BackgroundTracker.requestPermissions().then(res => {
      if (!res.backgroundGranted && !localStorage.getItem('geovixa_bg_prompted')) {
        localStorage.setItem('geovixa_bg_prompted', '1');
        showToast('For live tracking to keep working with the screen off, please allow "Location: Allow all the time" in the next screen.');
        window.Capacitor.Plugins.BackgroundTracker.openBackgroundLocationSettings();
      }
    }).catch(() => { /* permission flow failed — foreground-only tracking still works */ });
  }
}
...
function activateNavTab(tabKey) {
  ...
  if (tabKey === 'regularization') loadRegularization();
  if (tabKey === 'incidents') loadIncidents();
  if (tabKey === 'roster') { loadProjects(); loadRoster(); }
  if (tabKey === 'referrals') loadReferrals();
  if (tabKey === 'holidays') loadHolidays();
  if (tabKey === 'anomalies') loadAnomalies();
  ...
  if (tabKey === 'performance') { loadProjects(); loadPerformance(); }
  if (tabKey === 'predictive') loadPredictive();
  if (tabKey === 'ai-assistant') initAiAssistant();
...
function openEmployeeWorkspace(key) {
  ...
  if (addSel) {
    if (entry.members.length > 1) {
      ...
      addSel.value = '';
      loadSiteLocationDropdown('', null, 'add-emp-site-location');
    } else {
      addSel.innerHTML = `<option value="${entry.members[0]}">${entry.members[0]}</option>`;
      addSel.value = entry.members[0];
      loadSiteLocationDropdown(entry.members[0], null, 'add-emp-site-location');
    }
...
document.getElementById('add-emp-btn').addEventListener('click', async () => {
  ...
  const site_location_id = document.getElementById('add-emp-site-location').value || null;
  ...
  try {
    await apiFetch('/employees', {
      ...
      body: JSON.stringify({
        ...
        zone, ward, site_code, site_location_id,
    ...
    document.getElementById('add-emp-site-location').innerHTML = '<option value="">— No specific location (use project\'s geofence) —</option>';
...
// Populates a "Site Location" dropdown for whichever project is currently selected —
// shared by both the Add Employee form and the Edit Employee modal (targetSelectId
// lets one function serve both instead of duplicating this logic).
async function loadSiteLocationDropdown(project, selectedId, targetSelectId = 'edit-emp-site-location') {
  ...
  try {
    ...
    data.locations.forEach(l => {
      ...
      opt.value = l.id;
      opt.textContent = `${l.name} (${l.radius_m}m radius, ${l.employee_count} employee${l.employee_count === 1 ? '' : 's'})`;
...
document.getElementById('add-emp-project').addEventListener('change', (e) => {
  loadSiteLocationDropdown(e.target.value, null, 'add-emp-site-location');
});
...
function resetSiteLocationForm() {
  document.getElementById('site-loc-editing-id').value = '';
  ...
  document.getElementById('site-loc-form-title').textContent = '➕ Add one location';
  document.getElementById('site-loc-add-btn').textContent = '+ Add Location';
  document.getElementById('site-loc-cancel-edit-btn').classList.add('hidden');
}
    ...
    tbody.innerHTML = data.locations.map(l => `
      <tr${l.shortage > 0 ? ' style="background:#FEF2F2"' : ''}>
        ...
        <td style="white-space:nowrap;text-align:right;">
          <button class="btn small secondary" style="padding:4px 10px;" onclick='editSiteLocation(${JSON.stringify(l).replace(/'/g, "&apos;")})'>✏️ Edit</button>
          <button class="btn small secondary" style="padding:4px 10px;" onclick="deleteSiteLocation(${l.id}, '${project.replace(/'/g, "\\'")}')">🗑</button>
        </td>
      </tr>
    `).join('');
  } catch (err) {
...
// Populates the Add-location form with an existing location's values and switches the
// button into "Update" mode — previously the only way to change a location's coordinates
// or radius was to Delete it and re-add it from scratch, losing its edit history.
function editSiteLocation(loc) {
  document.getElementById('site-loc-editing-id').value = loc.id;
  document.getElementById('site-loc-add-name').value = loc.name;
  document.getElementById('site-loc-add-lat').value = loc.latitude;
  document.getElementById('site-loc-add-lng').value = loc.longitude;
  document.getElementById('site-loc-add-radius').value = loc.radius_m;
  document.getElementById('site-loc-add-required').value = loc.required_manpower;
  document.getElementById('site-loc-form-title').textContent = `✏️ Editing "${loc.name}"`;
  document.getElementById('site-loc-add-btn').textContent = '💾 Update Location';
  document.getElementById('site-loc-cancel-edit-btn').classList.remove('hidden');
  document.getElementById('site-loc-add-name').scrollIntoView({ behavior: 'smooth', block: 'center' });
}
document.getElementById('site-loc-cancel-edit-btn').addEventListener('click', resetSiteLocationForm);
...
document.getElementById('site-loc-add-btn').addEventListener('click', async () => {
  ...
  const editingId = document.getElementById('site-loc-editing-id').value;
  ...
  try {
    if (editingId) {
      await apiFetch(`/site-locations/${editingId}`, { method: 'PUT', body: JSON.stringify({ name, latitude, longitude, radius_m, required_manpower }) });
...
async function loadSosAlerts() {
  try {
    ...
    tbody.innerHTML = data.alerts.map(a => `
      <tr${a.escalated && a.status === 'open' ? ' style="background:#FEF2F2"' : ''}>
        ...
        <td>
          ...
          ${a.escalated && a.status === 'open' ? '<br><span class="badge critical" style="margin-top:4px;display:inline-block;">⚠️ OVERDUE — escalate now</span>' : ''}
        </td>
        <td>
...
// If running inside the Geovixa Android app (Capacitor), also hand off tracking to a native
// foreground service so it keeps running when the screen is off / app is minimized — this
// JS timer alone pauses in the background, which is fine for browser use but not enough for
// the app. In a normal browser, window.Capacitor is undefined and this is simply skipped.
function nativeTrackerAvailable() {
  return !!(window.Capacitor && window.Capacitor.isNativePlatform && window.Capacitor.isNativePlatform() && window.Capacitor.Plugins && window.Capacitor.Plugins.BackgroundTracker);
}
function startLiveLocationTracking() {
  if (nativeTrackerAvailable()) {
    try {
      window.Capacitor.Plugins.BackgroundTracker.start({ token: getToken(), apiBase: API });
    } catch (e) { /* fall through to JS-only tracking below */ }
  }
...
function stopLiveLocationTracking() {
  if (nativeTrackerAvailable()) {
    try { window.Capacitor.Plugins.BackgroundTracker.stop(); } catch (e) { /* ignore */ }
  }
...
let trackingRouteLayer = null; // separate layer for search-mode route line/stops
let trackingSearchMode = false;
let trackingEmployeesCache = null;
let trackingSelectedEmployeeId = null;
let trackingMapRefreshTimer = null;

function ensureTrackingMap() {
  if (trackingMapInstance) return trackingMapInstance;
  if (typeof L === 'undefined') {
    // Previously a failed Leaflet load (blocked CDN, offline, etc.) meant this whole tab
    // silently rendered nothing — no map, no error, nothing. Now self-hosted from
    // /vendor/leaflet/ (no external CDN dependency at all), but this guard stays as a
    // clear, visible diagnostic if the static files are ever missing/misconfigured.
    document.getElementById('tracking-map').innerHTML =
      '<div style="padding:40px 20px;text-align:center;color:var(--text-muted);font-size:13px;">⚠️ Map library failed to load (missing /vendor/leaflet/leaflet.js). Check that the backend is serving the public/vendor folder correctly.</div>';
  ...
  // Esri's free ArcGIS Online basemap tiles — no API key/signup required for this basic
  // usage level, unlike CARTO (which recently started requiring a key even for its free
  // tier — the "API KEY REQUIRED" watermark you saw) and raw OSM tile servers (which
  // outright blocked requests under their volunteer-run "no heavy automated use" policy).
  // Note Esri's tile URL order is {z}/{y}/{x}, not the usual {z}/{x}/{y}.
  L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Street_Map/MapServer/tile/{z}/{y}/{x}', {
    attribution: 'Tiles &copy; Esri &mdash; Source: Esri, DeLorme, NAVTEQ',
    maxZoom: 19,
  }).addTo(trackingMapInstance);
  trackingMapLayer = L.layerGroup().addTo(trackingMapInstance);
  trackingRouteLayer = L.layerGroup().addTo(trackingMapInstance);
  return trackingMapInstance;
}

async function loadTrackingMap() {
  if (trackingSearchMode) return; // a specific employee's route is being shown instead
  try {
    ...
    data.employees.forEach(e => {
      ...
      bounds.push(latlng);
      // Freshness check — a browser/WebView app stops sending pings the moment the
      // screen locks or the app is backgrounded (no real background service), so a
      // ping that's minutes/hours old is common and was previously shown as if it
      // were live. Color-code by age so admins can actually trust what they see:
      //   🟢 fresh (<5 min)   🟠 aging (5–20 min)   🔴 stale (>20 min, last known only)
      const pingAgeMs = e.live_last_ping_at ? (Date.now() - new Date(e.live_last_ping_at).getTime()) : Infinity;
      const pingAgeMin = pingAgeMs / 60000;
      let dot = '🟢', staleLabel = '';
      if (pingAgeMin > 20) { dot = '🔴'; staleLabel = ' — ⚠️ STALE (last known position only)'; }
      else if (pingAgeMin > 5) { dot = '🟠'; staleLabel = ' — aging'; }

      // Live battery % — only sent by the native Android app (browsers have no
      // reliable battery API), so this is null/blank for anyone still on the old
      // web-based app. Shown so admins get an early warning before a phone dies
      // and tracking silently stops.
      let batteryLabel = '';
      if (e.live_battery_percent !== null && e.live_battery_percent !== undefined) {
        const chargingIcon = e.live_is_charging ? '⚡' : '';
        const batteryIcon = e.live_battery_percent <= 15 ? '🪫' : '🔋';
        const lowWarning = e.live_battery_percent <= 15 && !e.live_is_charging
          ? ' <b style="color:#F0473C">LOW — may lose tracking soon</b>' : '';
        batteryLabel = `<br><span style="font-size:11px;color:#64748B">${batteryIcon} ${e.live_battery_percent}%${chargingIcon}${lowWarning}</span>`;
      }

      L.marker(latlng, {
        icon: L.divIcon({ className: '', html: dot, iconSize: [22, 22] }),
      }).addTo(trackingMapLayer).bindPopup(
        `<b>${escapeHtml(e.name)}</b> (${e.employee_id})<br>${escapeHtml(e.project || '')}<br><span style="font-size:11px;color:#64748B">Last ping: ${formatISTDateTime(e.live_last_ping_at)}${staleLabel}</span>${batteryLabel}`
      );
    });

    const staleCount = data.employees.filter(e => {
      const ageMin = e.live_last_ping_at ? (Date.now() - new Date(e.live_last_ping_at).getTime()) / 60000 : Infinity;
      return ageMin > 20;
    }).length;
    ...
    document.getElementById('tracking-last-updated').textContent =
      `Updated ${new Date().toLocaleTimeString('en-IN', { timeZone: 'Asia/Kolkata' })} — ${data.employees.length} on-duty with GPS (${staleCount} stale), ${data.sites.length} site(s) mapped`;

    // Previously an empty result just showed a blank grey map with no explanation —
    // easy to mistake for "the feature is broken" when really it's just that nobody
    // is on duty / no project has a location configured yet.
    document.getElementById('tracking-empty-note').classList.toggle('hidden', data.employees.length > 0 || data.sites.length > 0);
  } catch (err) {
    showToast(err.message, true);
    const container = document.getElementById('tracking-map');
    if (container && !trackingMapInstance) {
      // Map never even initialized (e.g. Leaflet missing, or the very first API call
      // failed) — leave a visible, permanent reason in the tab itself, not just a toast
      // that disappears after a few seconds and could be missed.
      container.innerHTML = `<div style="padding:40px 20px;text-align:center;color:var(--danger,#F0473C);font-size:13px;">⚠️ Could not load the tracking map: ${escapeHtml(err.message)}</div>`;
    }
  }
}
...
// ===========================================================================
// EMPLOYEE PERFORMANCE — was a fully built backend (GET /api/performance) with a ready
// HTML table, but no JS ever called it, so the tab always rendered empty. Wired here.
// ===========================================================================
async function loadPerformance() {
  const tbody = document.getElementById('perf-table-body');
  tbody.innerHTML = `<tr><td colspan="8"><div class="empty-state"><div class="icon">⏳</div>Loading…</div></td></tr>`;
  try {
    const project = document.getElementById('perf-filter-project').value;
    const data = await apiFetch(`/performance${project ? `?project=${encodeURIComponent(project)}` : ''}`);
    if (!data.employees.length) {
      tbody.innerHTML = `<tr><td colspan="8"><div class="empty-state"><div class="icon">🏆</div>No attendance history yet to score.</div></td></tr>`;
      return;
    }
    const catBadge = { 'Excellent': 'reliever_on_duty', 'Good': 'assigned', 'Average': 'critical', 'Needs Improvement': 'critical' };
    tbody.innerHTML = data.employees.map(e => `
      <tr>
        <td>${escapeHtml(e.name)} <span class="mono" style="font-size:11px;color:var(--text-muted)">(${e.employee_id})</span></td>
        <td>${escapeHtml(e.project || '')}</td>
        <td class="mono">${e.attendance_pct}%</td>
        <td class="mono">${e.on_time_pct}%</td>
        <td class="mono">${e.absences}</td>
        <td class="mono" style="font-weight:700;">${e.score}</td>
        <td><span class="badge ${catBadge[e.category] || ''}">${e.category}</span></td>
        <td style="white-space:nowrap;">
          <button class="btn small" style="padding:4px 10px;" onclick="openManagerReviewForm('${e.employee_id}', ${JSON.stringify(e.name).replace(/"/g, '&quot;')})">📝 Review</button>
          <button class="btn small secondary" style="padding:4px 10px;" onclick="viewManagerReviews('${e.employee_id}', ${JSON.stringify(e.name).replace(/"/g, '&quot;')})">🕘 History</button>
        </td>
      </tr>
    `).join('');
  } catch (err) {
    tbody.innerHTML = `<tr><td colspan="8"><div class="empty-state"><div class="icon">⚠️</div>${escapeHtml(err.message)}</div></td></tr>`;
  }
}
document.getElementById('perf-refresh-btn').addEventListener('click', loadPerformance);
document.getElementById('perf-filter-project').addEventListener('change', loadPerformance);

// ---- Manager review (qualitative rating + feedback) — additive to the auto-computed score
// above. Follows the same prompt()-based quick-input pattern already used throughout this
// file (see reviewRegularization/reviewIncident) rather than introducing a new modal system
// this codebase doesn't otherwise have.
async function openManagerReviewForm(employeeId, employeeName) {
  const periodLabel = prompt(`Review period for ${employeeName} (e.g. "Sep 2026"):`, new Date().toLocaleString('en-IN', { month: 'short', year: 'numeric' }));
  if (!periodLabel) return;
  const ratingRaw = prompt('Rating 1-5 (1 = Needs Improvement, 5 = Outstanding):', '4');
  const rating = Number(ratingRaw);
  if (!Number.isInteger(rating) || rating < 1 || rating > 5) { showToast('Rating must be a whole number from 1 to 5', true); return; }
  const strengths = prompt('Strengths (optional):', '') || '';
  const areas_to_improve = prompt('Areas to improve (optional):', '') || '';
  const feedback = prompt('Any other feedback (optional):', '') || '';
  try {
    await apiFetch('/performance/reviews', {
      method: 'POST',
      body: JSON.stringify({ employee_id: employeeId, period_label: periodLabel, rating, strengths, areas_to_improve, feedback }),
    });
    showToast(`Review saved for ${employeeName}`);
  } catch (err) { showToast(err.message, true); }
}

async function viewManagerReviews(employeeId, employeeName) {
  try {
    const data = await apiFetch(`/performance/reviews?employee_id=${encodeURIComponent(employeeId)}`);
    if (!data.reviews.length) { showToast(`No reviews yet for ${employeeName}`); return; }
    const lines = data.reviews.map(r =>
      `${new Date(r.created_at).toLocaleDateString('en-IN')} — ${r.period_label} — ${'★'.repeat(r.rating)}${'☆'.repeat(5 - r.rating)} by ${r.reviewed_by}` +
      (r.feedback ? `\n  "${r.feedback}"` : '')
    );
    alert(`Review history — ${employeeName}\n\n${lines.join('\n\n')}`);
  } catch (err) { showToast(err.message, true); }
}

// ===========================================================================
// PREDICTIVE WORKFORCE — same situation: GET /api/predictive/forecast existed and worked,
// nothing ever rendered it into #predictive-list.
// ===========================================================================
async function loadPredictive() {
  ...
  try {
    ...
    document.getElementById('predictive-method-label').textContent =
      data.method === 'ml' ? '🤖 ML model' : '📊 Heuristic (same-weekday history)';
    document.getElementById('predictive-forecast-date').textContent =
      `Forecasting shortage probability for ${data.forecast_for_date || 'tomorrow'}, based on each site's own history on the same weekday.`;

    if (!data.forecasts || !data.forecasts.length) {
      list.innerHTML = `<div class="empty-state"><div class="icon">🔮</div>No sites have a required-manpower target set yet, so there's nothing to forecast. Set one under a project's Site Details.</div>`;
      return;
    }
    list.innerHTML = data.forecasts.map(f => {
      const pct = f.shortage_probability === null ? null : Math.round(f.shortage_probability * 100);
      const barColor = pct === null ? '#CBD5E1' : pct >= 60 ? '#F0473C' : pct >= 30 ? '#F5A623' : '#12B76A';
      return `
        <div style="display:flex;align-items:center;gap:14px;padding:12px 4px;border-bottom:1px solid var(--border);">
          <div style="flex:1;min-width:0;">
            <div style="font-weight:600;">${escapeHtml(f.project)}</div>
            <div style="font-size:11.5px;color:var(--text-muted);margin-top:2px;">${escapeHtml(f.basis || '')}</div>
          </div>
          <div style="width:160px;flex-shrink:0;">
            <div style="background:#EEF0FB;border-radius:6px;height:8px;overflow:hidden;">
              <div style="width:${pct ?? 0}%;background:${barColor};height:100%;"></div>
            </div>
          </div>
          <div style="width:56px;text-align:right;font-weight:700;color:${barColor};flex-shrink:0;">${pct === null ? '—' : pct + '%'}</div>
        </div>`;
    }).join('');
  } catch (err) {
...
document.getElementById('predictive-refresh-btn').addEventListener('click', loadPredictive);

// ===========================================================================
// AI ASSISTANT — POST /api/ai-assistant/query existed and worked; the chat log/input
// were plain HTML with no event listener wired to them at all.
// ===========================================================================
let aiAssistantInitialized = false;
function initAiAssistant() {
  const log = document.getElementById('ai-chat-log');
  if (!aiAssistantInitialized) {
    log.innerHTML = `<div style="font-size:12.5px;color:var(--text-muted);padding:8px 0;">👋 Ask me things like "how many employees are absent today" or "which sites are critical".</div>`;
    aiAssistantInitialized = true;
  }
}
function appendAiMessage(text, fromUser) {
  const log = document.getElementById('ai-chat-log');
  const bubble = document.createElement('div');
  bubble.style.cssText = `margin:6px 0;padding:9px 13px;border-radius:10px;max-width:80%;font-size:13.5px;line-height:1.4;${
    fromUser ? 'background:#4F5FF0;color:#fff;margin-left:auto;text-align:right;' : 'background:#EEF0FB;color:var(--text);'
  }`;
...
async function sendAiQuestion() {
  ...
  input.disabled = true;
  try {
    const data = await apiFetch('/ai-assistant/query', { method: 'POST', body: JSON.stringify({ question }) });
    appendAiMessage(data.answer, false);
  } catch (err) {
    appendAiMessage(`⚠️ ${err.message}`, false);
  } finally {
    input.disabled = false;
    input.focus();
  }
}
document.getElementById('ai-chat-send-btn').addEventListener('click', sendAiQuestion);
document.getElementById('ai-chat-input').addEventListener('keydown', (e) => {
  if (e.key === 'Enter') sendAiQuestion();
});

// ===========================================================================
// ATTENDANCE CORRECTIONS (regularization) — was submittable from the app but had
// ZERO admin UI to approve/reject it. Approving writes a real attendance row.
// ===========================================================================
async function loadRegularization() {
  const tbody = document.getElementById('regularization-table-body');
  tbody.innerHTML = `<tr><td colspan="10"><div class="empty-state"><div class="icon">⏳</div>Loading…</div></td></tr>`;
  try {
    const status = document.getElementById('regularization-status-filter').value;
    const data = await apiFetch(`/regularization${status ? `?status=${status}` : ''}`);
    if (!data.requests.length) {
      tbody.innerHTML = `<tr><td colspan="10"><div class="empty-state"><div class="icon">✏️</div>No correction requests.</div></td></tr>`;
      return;
    }
    tbody.innerHTML = data.requests.map(r => `
      <tr>
        <td class="bulk-select-col">${r.status === 'pending' ? `<input type="checkbox" class="row-select" data-bulk-table="regularization" value="${r.id}" />` : ''}</td>
        <td class="mono">${r.employee_id}</td>
        <td>${escapeHtml(r.employee_name || '')}</td>
        <td class="mono">${r.attendance_date}</td>
        <td>${r.requested_status === 'on_duty' ? 'Punch In' : 'Punch Out'}</td>
        <td class="mono">${r.requested_time || '-'}</td>
        <td style="font-size:12px">${escapeHtml(r.reason || '')}</td>
        <td><span class="badge ${r.status}">${r.status}</span></td>
        <td class="mono" style="font-size:12px">${formatISTDateTime(r.created_at)}</td>
        <td style="white-space:nowrap;">
          ${r.status === 'pending' ? `
            <button class="btn small" style="padding:4px 10px;" onclick="reviewRegularization(${r.id}, 'approved')">✔ Approve</button>
            <button class="btn small secondary" style="padding:4px 10px;" onclick="reviewRegularization(${r.id}, 'rejected')">✘ Reject</button>
          ` : '-'}
        </td>
      </tr>
    `).join('');
    if (typeof refreshBulkBar === 'function') refreshBulkBar('regularization');
  } catch (err) { showToast(err.message, true); }
}
async function reviewRegularization(id, status) {
  const note = status === 'rejected' ? (prompt('Reason for rejecting (optional):') || '') : '';
  try {
    await apiFetch(`/regularization/${id}/${status === 'approved' ? 'approve' : 'reject'}`, { method: 'PUT', body: JSON.stringify({ note }) });
    showToast(status === 'approved' ? 'Approved' : 'Rejected');
    loadRegularization();
  } catch (err) { showToast(err.message, true); }
}
document.getElementById('regularization-refresh-btn').addEventListener('click', loadRegularization);
document.getElementById('regularization-status-filter').addEventListener('change', loadRegularization);

// ===========================================================================
// INCIDENT / DUTY REPORTS — same situation: submittable from the app, no admin UI.
// ===========================================================================
async function loadIncidents() {
  const tbody = document.getElementById('incidents-table-body');
  tbody.innerHTML = `<tr><td colspan="10"><div class="empty-state"><div class="icon">⏳</div>Loading…</div></td></tr>`;
  try {
    const status = document.getElementById('incidents-status-filter').value;
    const data = await apiFetch(`/incidents${status ? `?status=${status}` : ''}`);
    if (!data.reports.length) {
      tbody.innerHTML = `<tr><td colspan="10"><div class="empty-state"><div class="icon">🚧</div>No incident reports.</div></td></tr>`;
      return;
    }
    const sevBadge = { high: 'critical', medium: 'assigned', low: '' };
    tbody.innerHTML = data.reports.map(r => `
      <tr>
        <td class="bulk-select-col">${r.status !== 'resolved' ? `<input type="checkbox" class="row-select" data-bulk-table="incidents" value="${r.id}" />` : ''}</td>
        <td class="mono">${r.employee_id}</td>
        <td>${escapeHtml(r.employee_name || '')}</td>
        <td>${r.category.replace('_', ' ')}</td>
        <td><span class="badge ${sevBadge[r.severity] || ''}">${r.severity}</span></td>
        <td style="font-size:12px;max-width:220px;">${escapeHtml(r.description)}</td>
        <td>${r.photo_url ? `<a href="${r.photo_url}" target="_blank">📷 View</a>` : '-'}</td>
        <td><span class="badge ${r.status}">${r.status}</span></td>
        <td class="mono" style="font-size:12px">${formatISTDateTime(r.created_at)}</td>
        <td style="white-space:nowrap;">
          ${r.status !== 'resolved' ? `
            <button class="btn small" style="padding:4px 10px;" onclick="reviewIncident(${r.id}, 'reviewed')">👁 Reviewed</button>
            <button class="btn small secondary" style="padding:4px 10px;" onclick="reviewIncident(${r.id}, 'resolved')">✔ Resolve</button>
          ` : '-'}
        </td>
      </tr>
    `).join('');
    if (typeof refreshBulkBar === 'function') refreshBulkBar('incidents');
  } catch (err) { showToast(err.message, true); }
}
async function reviewIncident(id, status) {
  const note = status === 'resolved' ? (prompt('Resolution note (optional):') || '') : '';
  try {
    await apiFetch(`/incidents/${id}/review`, { method: 'PUT', body: JSON.stringify({ status, resolution_note: note }) });
    showToast('Updated');
    loadIncidents();
  } catch (err) { showToast(err.message, true); }
}
document.getElementById('incidents-refresh-btn').addEventListener('click', loadIncidents);
document.getElementById('incidents-status-filter').addEventListener('change', loadIncidents);

// ===========================================================================
// SHIFT ROSTER — employees could see "My Schedule" in the app, but nobody could ever
// actually assign one. This is the missing other half.
// ===========================================================================
async function loadRoster() {
  const tbody = document.getElementById('roster-table-body');
  tbody.innerHTML = `<tr><td colspan="8"><div class="empty-state"><div class="icon">⏳</div>Loading…</div></td></tr>`;
  try {
    const from = document.getElementById('roster-from-date').value;
    const to = document.getElementById('roster-to-date').value;
    const project = document.getElementById('roster-project-filter').value;
    const params = new URLSearchParams();
    if (from) params.set('from', from);
    if (to) params.set('to', to);
    if (project) params.set('project', project);
    const data = await apiFetch(`/roster?${params.toString()}`);
    if (!data.roster.length) {
      tbody.innerHTML = `<tr><td colspan="8"><div class="empty-state"><div class="icon">🗓️</div>No shifts scheduled in this range.</div></td></tr>`;
      return;
    }
    tbody.innerHTML = data.roster.map(r => `
      <tr>
        <td class="mono">${r.roster_date}</td>
        <td>${escapeHtml(r.employee_name || r.employee_id)}</td>
        <td>${escapeHtml(r.project || '-')}</td>
        <td>${escapeHtml(r.site_name || '-')}</td>
        <td>${escapeHtml(r.shift_category || '-')}</td>
        <td class="mono">${r.shift_start_time || '-'}</td>
        <td><span class="badge ${r.status}">${r.status}</span></td>
        <td><button class="btn small secondary" style="padding:4px 10px;" onclick="deleteRosterEntry(${r.id})">🗑</button></td>
      </tr>
    `).join('');
  } catch (err) { showToast(err.message, true); }
}
document.getElementById('roster-add-btn').addEventListener('click', async () => {
  const employee_id = document.getElementById('roster-add-employee-id').value.trim();
  const roster_date = document.getElementById('roster-add-date').value;
  const project = document.getElementById('roster-add-project').value;
  const shift_category = document.getElementById('roster-add-shift-category').value.trim();
  const shift_start_time = document.getElementById('roster-add-shift-time').value;
  if (!employee_id || !roster_date) { showToast('Employee ID and date are required', true); return; }
  try {
    await apiFetch('/roster', { method: 'POST', body: JSON.stringify({ employee_id, project, roster_date, shift_category, shift_start_time }) });
    showToast('Shift assigned');
    document.getElementById('roster-add-employee-id').value = '';
    document.getElementById('roster-add-shift-category').value = '';
    loadRoster();
  } catch (err) { showToast(err.message, true); }
});
async function deleteRosterEntry(id) {
  if (!confirm('Remove this roster entry?')) return;
  try { await apiFetch(`/roster/${id}`, { method: 'DELETE' }); showToast('Removed'); loadRoster(); }
  catch (err) { showToast(err.message, true); }
}
document.getElementById('roster-refresh-btn').addEventListener('click', loadRoster);
...
async function loadReferrals() {
  ...
  tbody.innerHTML = `<tr><td colspan="10"><div class="empty-state"><div class="icon">⏳</div>Loading…</div></td></tr>`;
  try {
    const status = document.getElementById('referrals-status-filter').value;
    const data = await apiFetch(`/referrals${status ? `?status=${status}` : ''}`);
    if (!data.referrals.length) {
      tbody.innerHTML = `<tr><td colspan="10"><div class="empty-state"><div class="icon">👥</div>No referrals yet.</div></td></tr>`;
      return;
    }
    tbody.innerHTML = data.referrals.map(r => `
      <tr>
        <td class="bulk-select-col">${r.status === 'pending' ? `<input type="checkbox" class="row-select" data-bulk-table="referrals" value="${r.id}" />` : ''}</td>
        <td>${escapeHtml(r.referrer_name || r.referrer_employee_id)}</td>
        <td>${escapeHtml(r.referred_name)}</td>
        <td class="mono">${escapeHtml(r.referred_phone)}</td>
        <td style="font-size:12px">${escapeHtml(r.note || '-')}</td>
        <td><span class="badge ${r.status}">${r.status}</span></td>
        <td class="mono">${r.reward_amount != null ? '₹' + r.reward_amount : '-'}</td>
        <td>${r.reward_paid ? '✅' : '-'}</td>
        <td class="mono" style="font-size:12px">${formatISTDateTime(r.created_at)}</td>
        <td style="white-space:nowrap;">
          ${r.status === 'pending' ? `
            <button class="btn small" style="padding:4px 10px;" onclick="reviewReferral(${r.id}, 'hired')">✔ Hired</button>
            <button class="btn small secondary" style="padding:4px 10px;" onclick="reviewReferral(${r.id}, 'rejected')">✘ Reject</button>
          ` : `<button class="btn small secondary" style="padding:4px 10px;" onclick="markReferralPaid(${r.id})">💰 Mark Paid</button>`}
        </td>
      </tr>
    `).join('');
    if (typeof refreshBulkBar === 'function') refreshBulkBar('referrals');
  } catch (err) { showToast(err.message, true); }
}
async function reviewReferral(id, status) {
  let reward_amount;
  if (status === 'hired') reward_amount = prompt('Reward amount (₹, optional):') || undefined;
  try {
    await apiFetch(`/referrals/${id}`, { method: 'PUT', body: JSON.stringify({ status, reward_amount }) });
    showToast('Updated');
    loadReferrals();
  } catch (err) { showToast(err.message, true); }
}
async function markReferralPaid(id) {
  try { await apiFetch(`/referrals/${id}`, { method: 'PUT', body: JSON.stringify({ reward_paid: true }) }); showToast('Marked paid'); loadReferrals(); }
  catch (err) { showToast(err.message, true); }
}
document.getElementById('referrals-refresh-btn').addEventListener('click', loadReferrals);
document.getElementById('referrals-status-filter').addEventListener('change', loadReferrals);
...
async function loadHolidays() {
  ...
  tbody.innerHTML = `<tr><td colspan="3"><div class="empty-state"><div class="icon">⏳</div>Loading…</div></td></tr>`;
  try {
    const year = document.getElementById('holidays-year-filter').value || new Date().getFullYear();
    const data = await apiFetch(`/holidays?year=${year}`);
    if (!data.holidays.length) {
      tbody.innerHTML = `<tr><td colspan="3"><div class="empty-state"><div class="icon">🎉</div>No holidays added for ${year}.</div></td></tr>`;
      return;
    }
    tbody.innerHTML = data.holidays.map(h => `
      <tr>
        <td class="mono">${h.holiday_date}</td>
        <td>${escapeHtml(h.name)}</td>
        <td><button class="btn small secondary" style="padding:4px 10px;" onclick="deleteHoliday(${h.id})">🗑</button></td>
      </tr>
    `).join('');
  } catch (err) { showToast(err.message, true); }
}
document.getElementById('holiday-add-btn').addEventListener('click', async () => {
  const holiday_date = document.getElementById('holiday-add-date').value;
  const name = document.getElementById('holiday-add-name').value.trim();
  if (!holiday_date || !name) { showToast('Date and name are required', true); return; }
  try {
    await apiFetch('/holidays', { method: 'POST', body: JSON.stringify({ holiday_date, name }) });
...
async function deleteHoliday(id) {
  ...
  try { await apiFetch(`/holidays/${id}`, { method: 'DELETE' }); showToast('Removed'); loadHolidays(); }
  catch (err) { showToast(err.message, true); }
}
document.getElementById('holidays-refresh-btn').addEventListener('click', loadHolidays);

// ===========================================================================
// LEAVE BALANCE — admin sets allocated_days per employee/year. Previously
// GET /leave/my/balance always fell back to a hardcoded default of 12 because
// nothing in the admin panel could ever write to leave_balances.
// ===========================================================================
document.getElementById('leave-balance-save-btn').addEventListener('click', async () => {
  const employee_id = document.getElementById('leave-balance-employee-id').value.trim();
  const year = document.getElementById('leave-balance-year').value || new Date().getFullYear();
  const allocated_days = document.getElementById('leave-balance-days').value;
  const resultEl = document.getElementById('leave-balance-result');
  if (!employee_id || !allocated_days) { resultEl.textContent = 'Employee ID and days are required'; return; }
  try {
    await apiFetch(`/leave/balance/${employee_id}`, { method: 'PUT', body: JSON.stringify({ year, allocated_days }) });
    resultEl.textContent = `✅ Saved ${allocated_days} days for ${employee_id} (${year})`;
    resultEl.style.color = 'var(--success, #12B76A)';
  } catch (err) {
    resultEl.textContent = `❌ ${err.message}`;
    resultEl.style.color = 'var(--danger, #F0473C)';
  }
});

// ===========================================================================
// GPS ANOMALY DETECTION — the backend already rejects impossible-travel punches
// server-side and logs them to gps_anomaly_flags, but there was no admin visibility
// into these flags at all before this dashboard.
// ===========================================================================
async function loadAnomalies() {
  const tbody = document.getElementById('anomalies-table-body');
  tbody.innerHTML = `<tr><td colspan="8"><div class="empty-state"><div class="icon">⏳</div>Loading…</div></td></tr>`;
  try {
    const reviewed = document.getElementById('anomalies-reviewed-filter').value;
    const data = await apiFetch(`/attendance/anomalies${reviewed !== '' ? `?reviewed=${reviewed}` : ''}`);
    if (!data.anomalies.length) {
      tbody.innerHTML = `<tr><td colspan="8"><div class="empty-state"><div class="icon">✅</div>No flagged anomalies.</div></td></tr>`;
      return;
    }
    tbody.innerHTML = data.anomalies.map(a => `
      <tr${!a.reviewed ? ' style="background:#FEF2F2"' : ''}>
        <td class="mono">${a.employee_id}</td>
        <td>${escapeHtml(a.employee_name || '')}</td>
        <td>${escapeHtml(a.project || '-')}</td>
        <td class="mono">${Math.round(a.speed_kmh)} km/h</td>
        <td class="mono">${a.distance_km.toFixed(1)} km</td>
        <td class="mono">${Math.round(a.elapsed_minutes)} min</td>
        <td class="mono" style="font-size:12px">${formatISTDateTime(a.created_at)}</td>
        <td>${!a.reviewed ? `<button class="btn small secondary" style="padding:4px 10px;" onclick="markAnomalyReviewed(${a.id})">✔ Mark Reviewed</button>` : '✔ Reviewed'}</td>
      </tr>
    `).join('');
  } catch (err) { showToast(err.message, true); }
}
async function markAnomalyReviewed(id) {
  try { await apiFetch(`/attendance/anomalies/${id}/mark-reviewed`, { method: 'PUT' }); showToast('Marked reviewed'); loadAnomalies(); }
  catch (err) { showToast(err.message, true); }
}
document.getElementById('anomalies-refresh-btn').addEventListener('click', loadAnomalies);
document.getElementById('anomalies-reviewed-filter').addEventListener('change', loadAnomalies);

// ===========================================================================
// EMPLOYEE TRACKING — SEARCH + DAILY ROUTE HISTORY
// Search an employee by ID/name, pick a date, and see their full on-duty → off-duty
// route drawn as a line on the map (built from location_pings — GET /attendance/
// tracking-history). Right-clicking anywhere on the line shows that exact spot's
// address, arrival time, departure time, and how long they stayed there.
// ===========================================================================
async function ensureTrackingEmployeesCache() {
  if (trackingEmployeesCache) return trackingEmployeesCache;
  try {
    const data = await apiFetch('/employees');
    trackingEmployeesCache = data.employees || [];
  } catch (err) {
    trackingEmployeesCache = [];
  }
  return trackingEmployeesCache;
}

document.getElementById('tracking-search-date').value = new Date().toISOString().slice(0, 10);

document.getElementById('tracking-search-input').addEventListener('input', async (e) => {
  const q = e.target.value.trim().toLowerCase();
  const resultsBox = document.getElementById('tracking-search-results');
  trackingSelectedEmployeeId = null;
  if (q.length < 2) { resultsBox.classList.add('hidden'); resultsBox.innerHTML = ''; return; }

  const employees = await ensureTrackingEmployeesCache();
  const matches = employees.filter(emp =>
    emp.employee_id.toLowerCase().includes(q) || (emp.name || '').toLowerCase().includes(q)
  ).slice(0, 8);

  if (!matches.length) {
    resultsBox.innerHTML = `<div style="padding:10px;font-size:12.5px;color:var(--text-muted);">No employees match "${escapeHtml(e.target.value)}"</div>`;
    resultsBox.classList.remove('hidden');
    return;
  }
  resultsBox.innerHTML = matches.map(emp => `
    <div class="tracking-search-result-row" data-emp-id="${emp.employee_id}" style="padding:10px 12px;cursor:pointer;border-bottom:1px solid var(--border);font-size:13px;">
      <b>${emp.employee_id}</b> — ${escapeHtml(emp.name)} <span style="color:var(--text-muted);font-size:11.5px;">${escapeHtml(emp.project || '')}</span>
    </div>
  `).join('');
  resultsBox.classList.remove('hidden');
  resultsBox.querySelectorAll('.tracking-search-result-row').forEach(row => {
    row.addEventListener('click', () => {
      const emp = matches.find(m => m.employee_id === row.dataset.empId);
      trackingSelectedEmployeeId = emp.employee_id;
      document.getElementById('tracking-search-input').value = `${emp.employee_id} — ${emp.name}`;
...
document.getElementById('tracking-search-btn').addEventListener('click', async () => {
  let employeeId = trackingSelectedEmployeeId;
  if (!employeeId) {
    // Fallback: treat the raw typed text as an exact Employee ID if no dropdown pick was made.
    const raw = document.getElementById('tracking-search-input').value.trim();
    employeeId = raw.split('—')[0].trim();
  }
  if (!employeeId) { showToast('Type an Employee ID or Name first', true); return; }
  const date = document.getElementById('tracking-search-date').value || new Date().toISOString().slice(0, 10);
  await showEmployeeRoute(employeeId, date);
});

async function showEmployeeRoute(employeeId, date) {
  const summaryEl = document.getElementById('tracking-route-summary');
  summaryEl.textContent = 'Loading route…';
  try {
    const data = await apiFetch(`/attendance/tracking-history?employee_id=${encodeURIComponent(employeeId)}&date=${date}`);
    trackingSearchMode = true;
    stopTrackingMapAutoRefresh();
    document.getElementById('tracking-empty-note').classList.add('hidden');
    document.getElementById('tracking-search-clear-btn').classList.remove('hidden');
    document.getElementById('tracking-route-hint').classList.toggle('hidden', data.path.length === 0);

    const map = ensureTrackingMap();
    trackingMapLayer.clearLayers();
    trackingRouteLayer.clearLayers();

    if (!data.path.length) {
      summaryEl.textContent = `${data.employee.name} (${data.employee.employee_id}) — no GPS pings recorded on ${date}. (Only recorded while on duty using the Android app.)`;
      return;
    }

    const latlngs = data.path.map(p => [Number(p.latitude), Number(p.longitude)]);
    const routeLine = L.polyline(latlngs, { color: '#4F5FF0', weight: 4, opacity: 0.85 }).addTo(trackingRouteLayer);

    // Right-click anywhere on the line → find the nearest recorded stop (if the click
    // landed near one) or otherwise the nearest raw ping, and show its exact info.
    routeLine.on('contextmenu', (evt) => {
      L.DomEvent.preventDefault(evt.originalEvent);
      showRoutePointInfo(evt.latlng, data);
    });

    // Start/end markers
    L.marker(latlngs[0], { icon: L.divIcon({ className: '', html: '🟢', iconSize: [20, 20] }) })
      .addTo(trackingRouteLayer).bindPopup(`Shift start${data.shift.punch_in ? ' — ' + formatISTDateTime(data.shift.punch_in) : ''}`);
    L.marker(latlngs[latlngs.length - 1], { icon: L.divIcon({ className: '', html: data.shift.punch_out ? '🔴' : '🟠', iconSize: [20, 20] }) })
      .addTo(trackingRouteLayer).bindPopup(data.shift.punch_out ? `Shift end — ${formatISTDateTime(data.shift.punch_out)}` : 'Still on duty (last known position)');

    // Numbered stop markers — each one is directly right-clickable/clickable for its
    // own arrived/left/duration, same info the route-line right-click also surfaces.
    data.stops.forEach((stop, i) => {
      const marker = L.marker([stop.latitude, stop.longitude], {
        icon: L.divIcon({ className: '', html: `<div style="background:#F5A623;color:#fff;border-radius:50%;width:22px;height:22px;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;border:2px solid #fff;box-shadow:0 1px 4px rgba(0,0,0,0.3);">${i + 1}</div>`, iconSize: [22, 22] }),
      }).addTo(trackingRouteLayer);
      marker.bindPopup(buildStopPopupHtml(stop));
      marker.on('contextmenu', (evt) => { L.DomEvent.preventDefault(evt.originalEvent); marker.openPopup(); });
    });

    map.fitBounds(routeLine.getBounds(), { padding: [40, 40], maxZoom: 17 });

    const stopsText = data.stops.length
      ? `${data.stops.length} stop${data.stops.length === 1 ? '' : 's'} detected`
      : 'no significant stops detected (kept moving)';
    summaryEl.innerHTML = `<b>${escapeHtml(data.employee.name)}</b> (${data.employee.employee_id}) — ${date}<br>
      Shift: ${data.shift.punch_in ? formatISTDateTime(data.shift.punch_in) : '?'} → ${data.shift.punch_out ? formatISTDateTime(data.shift.punch_out) : 'still on duty'}
      &nbsp;·&nbsp; ${data.path.length} GPS points &nbsp;·&nbsp; ${stopsText}`;
  } catch (err) {
...
function buildStopPopupHtml(stop) {
  const stillHere = stop.still_here ? ' <b style="color:#F0473C">(still here)</b>' : '';
  return `
    <div style="font-size:12.5px;min-width:180px;">
      <div id="stop-address-${stop.arrived_at}" style="margin-bottom:6px;color:var(--text-muted,#64748B);">Looking up address…</div>
      <div><b>Arrived:</b> ${formatISTDateTime(stop.arrived_at)}</div>
      <div><b>${stop.still_here ? 'Last seen' : 'Left'}:</b> ${formatISTDateTime(stop.left_at)}${stillHere}</div>
      <div><b>Duration:</b> ${stop.duration_minutes} min</div>
    </div>
  `;
}

// Right-clicking a raw point on the route line (not exactly on a detected stop) shows the
// nearest ping's timestamp + a live reverse-geocoded address — geocoding happens lazily,
// only for the point actually clicked, instead of geocoding the whole day's path upfront.
async function showRoutePointInfo(latlng, data) {
  // If the click landed close to a detected stop, prefer showing that stop's full
  // arrived/left/duration instead of a single raw point.
  let nearestStop = null, nearestStopDist = Infinity;
  data.stops.forEach(stop => {
    const d = latlng.distanceTo(L.latLng(stop.latitude, stop.longitude));
    if (d < nearestStopDist) { nearestStopDist = d; nearestStop = stop; }
  });

  if (nearestStop && nearestStopDist < 80) {
    const popup = L.popup().setLatLng(latlng).setContent(buildStopPopupHtml(nearestStop)).openOn(trackingMapInstance);
    reverseGeocodeForPopup(nearestStop.latitude, nearestStop.longitude, `stop-address-${nearestStop.arrived_at}`);
    return;
  }

  // Otherwise, find the nearest raw ping to the right-click point.
  let nearestPing = null, nearestPingDist = Infinity;
  data.path.forEach(p => {
    const d = latlng.distanceTo(L.latLng(Number(p.latitude), Number(p.longitude)));
    if (d < nearestPingDist) { nearestPingDist = d; nearestPing = p; }
  });
  if (!nearestPing) return;

  const addressElId = `ping-address-${nearestPing.recorded_at}`;
  L.popup().setLatLng(latlng).setContent(`
    <div style="font-size:12.5px;min-width:180px;">
      <div id="${addressElId}" style="margin-bottom:6px;color:var(--text-muted,#64748B);">Looking up address…</div>
      <div><b>Recorded at:</b> ${formatISTDateTime(nearestPing.recorded_at)}</div>
      <div style="font-size:11px;color:var(--text-muted,#64748B);margin-top:4px;">${Number(nearestPing.latitude).toFixed(5)}, ${Number(nearestPing.longitude).toFixed(5)}</div>
    </div>
  `).openOn(trackingMapInstance);
  reverseGeocodeForPopup(nearestPing.latitude, nearestPing.longitude, addressElId);
}

async function reverseGeocodeForPopup(lat, lng, elementId) {
  try {
    const controller = new AbortController();
    ...
    if (el) el.textContent = json.display_name || `${Number(lat).toFixed(5)}, ${Number(lng).toFixed(5)}`;
  } catch (err) {
    const el = document.getElementById(elementId);
...
document.getElementById('tracking-search-clear-btn').addEventListener('click', () => {
  trackingSearchMode = false;
  trackingSelectedEmployeeId = null;
  document.getElementById('tracking-search-input').value = '';
  document.getElementById('tracking-search-results').classList.add('hidden');
  document.getElementById('tracking-route-summary').textContent = '';
  document.getElementById('tracking-route-hint').classList.add('hidden');
  document.getElementById('tracking-search-clear-btn').classList.add('hidden');
  if (trackingRouteLayer) trackingRouteLayer.clearLayers();
  loadTrackingMap();
  startTrackingMapAutoRefresh();
});

// Hide the search-results dropdown when clicking elsewhere on the page.
document.addEventListener('click', (e) => {
  const box = document.getElementById('tracking-search-results');
