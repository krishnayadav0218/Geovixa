...
// Device binding — a client-generated (not hardware) fingerprint, created once and reused
// forever on this browser/device. Sent with every punch so the backend can flag punches
// coming from an employee's account on an unfamiliar device (see routes/attendance.js).
function getDeviceId() {
  let id = safeLocalStorageGet('geovixa_device_id');
  if (!id) {
    id = 'dev_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2, 12);
    safeLocalStorageSet('geovixa_device_id', id);
  }
  return id;
}
...
async function captureAndSubmit() {
  ...
  try {
    await apiFetch('/attendance/punch', { method: 'POST', body: JSON.stringify(punchPayload) });
    ...
    // A genuine NETWORK failure (offline, DNS down, request never reached the server) throws
    // before any HTTP response exists — that's the one case where queueing for later makes
    // sense. A real server-side rejection (geofence, invalid photo, GPS validation, device
    // mismatch, etc.) already has a specific answer and would just fail again identically on
    // retry, so those are shown as errors exactly as before, never queued.
    if (isLikelyNetworkFailure(err)) {
      queueOfflinePunch(punchPayload);
      showToast('No connection — attendance saved on this device and will sync automatically once you\'re back online.');
      closeCameraModal();
      loadMyStatus(true);
      return;
    }
...
function activateNavTab(tabKey) {
  ...
  if (tabKey === 'ai-assistant') { /* nothing to preload — chat starts empty */ }
...
// ===========================================================================
// OFFLINE ATTENDANCE SYNC — a punch made with no network connectivity is queued on-device
// (localStorage) instead of lost, then automatically resubmitted the moment connectivity
// returns (the browser's 'online' event) or on a periodic retry timer as a backup. Never
// blocks the employee from marking attendance just because their signal dropped.
// ===========================================================================
function isLikelyNetworkFailure(err) {
  // A real HTTP error response (4xx/5xx from apiFetch) always has a specific server-given
  // message. A genuine network failure never reaches that point — fetch() itself rejects,
  // and browsers are fairly consistent about how: TypeError with "fetch"/"network" in the
  // message, or the browser already knows it's offline via navigator.onLine.
  if (!navigator.onLine) return true;
  return err instanceof TypeError || /fetch|network/i.test(err.message || '');
}

function getOfflineQueue() {
  try { return JSON.parse(safeLocalStorageGet('geovixa_offline_punches') || '[]'); } catch { return []; }
}
function setOfflineQueue(queue) {
  return safeLocalStorageSet('geovixa_offline_punches', JSON.stringify(queue));
}

function queueOfflinePunch(payload) {
  const queue = getOfflineQueue();
  queue.push({ ...payload, captured_at: payload.device_time, synced_late: true, queued_at: new Date().toISOString() });
  if (!setOfflineQueue(queue)) {
    // Storage genuinely full/unavailable — surfaced loudly rather than silently losing the
    // punch, since there's no queue to fall back to at that point.
    showToast('Could not save attendance offline (device storage full/blocked). Please try again once you have signal.', true);
    return;
  }
  updateOfflineSyncBadge();
}
...
async function trySyncOfflineQueue() {
  ...
  for (let i = 0; i < queue.length; i++) {
    const item = queue[i];
    try {
      await apiFetch('/attendance/punch', { method: 'POST', body: JSON.stringify(item) });
      syncedCount++;
    } catch (err) {
      if (isLikelyNetworkFailure(err)) {
        // Still offline — keep this one and everything after it queued, stop trying the rest this pass.
        remaining.push(...queue.slice(i));
        break;
      }
      // A real rejection this time (e.g. geofence/device issues) — drop it rather than retry
      // forever, but tell the employee so they know it didn't silently vanish successfully.
      showToast(`An offline attendance record could not be synced: ${err.message}`, true);
    }
  }
  setOfflineQueue(remaining);
  updateOfflineSyncBadge();
  if (syncedCount > 0) {
    showToast(`${syncedCount} offline attendance record(s) synced successfully`);
    loadMyStatus(true);
  }
}
...
setInterval(trySyncOfflineQueue, 30000); // backup retry even if the 'online' event is missed
...
async function loadPerformance() {
  try {
    const select = document.getElementById('perf-filter-project');
    if (!select.dataset.loaded) {
      const data = await apiFetch('/projects');
      data.projects.forEach(p => {
        const opt = document.createElement('option');
    ...
    if (!data.employees.length) {
      tbody.innerHTML = `<tr><td colspan="7"><div class="empty-state"><div class="icon">🏆</div>No employees to score</div></td></tr>`;
      return;
    }
    const categoryBadge = { 'Excellent': 'reliever_on_duty', 'Good': 'on_regular_duty', 'Average': 'reliever_pending', 'Needs Improvement': 'critical' };
    tbody.innerHTML = data.employees.map(e => `
      <tr>
        <td class="mono">${e.employee_id} — ${escapeHtml(e.name)}</td>
        <td>${escapeHtml(e.project || '-')}</td>
        ...
        <td style="font-weight:800;">${e.score}</td>
...
async function loadPredictive() {
  try {
    ...
    document.getElementById('predictive-method-label').textContent =
      data.method === 'ml' ? '🧠 ML forecast' : '📐 Formula-based forecast (Python ML unavailable)';
    document.getElementById('predictive-forecast-date').textContent = `Forecast for ${data.forecast_for_date} — based on each site's own history on the same weekday.`;
    const list = document.getElementById('predictive-list');
    if (!data.forecasts.length) {
      list.innerHTML = `<div class="empty-state"><div class="icon">🔮</div>No sites have a required-manpower target set yet, so nothing to forecast.</div>`;
      return;
    }
    list.innerHTML = data.forecasts.map(f => {
      if (f.shortage_probability == null) {
        return `<div class="panel" style="margin-bottom:8px;padding:10px 14px;">
          <b>${escapeHtml(f.project)}</b>
          <div style="font-size:12px;color:var(--text-muted);margin-top:4px;">${escapeHtml(f.basis)}</div>
        </div>`;
      }
      const pct = Math.round(f.shortage_probability * 100);
      const color = pct >= 60 ? '#B91C1C' : pct >= 30 ? '#B45309' : '#15803D';
      return `<div class="panel" style="margin-bottom:8px;padding:10px 14px;">
        <div style="display:flex;justify-content:space-between;align-items:center;">
          <div>
            <b>${escapeHtml(f.project)}</b>
            <div style="font-size:12px;color:var(--text-muted);margin-top:4px;">${escapeHtml(f.basis)} (confidence: ${f.confidence})</div>
          </div>
          <div style="font-weight:800;font-size:20px;color:${color};">${pct}%</div>
        </div>
      </div>`;
    }).join('');
  } catch (err) {
...
// AI ASSISTANT
// ===========================================================================
function appendAiChatMessage(text, isUser) {
  const log = document.getElementById('ai-chat-log');
  const bubble = document.createElement('div');
  bubble.style.cssText = `margin-bottom:10px;padding:10px 14px;border-radius:10px;max-width:80%;font-size:13px;${
    isUser ? 'background:var(--accent,#0B93D6);color:#fff;margin-left:auto;' : 'background:var(--bg,#F1F5F9);color:#0F172A;'
  }`;
...
async function sendAiChatMessage() {
  ...
  if (!question) return;
  appendAiChatMessage(question, true);
  input.value = '';
  try {
    const data = await apiFetch('/ai-assistant/query', { method: 'POST', body: JSON.stringify({ question }) });
    appendAiChatMessage(data.answer, false);
  } catch (err) {
    appendAiChatMessage('Sorry, something went wrong: ' + err.message, false);
  }
}
