...
function ensureTrackingMap() {
  ...
  // CARTO's free basemap CDN, not raw tile.openstreetmap.org — OSM's volunteer-run tile
  // servers had blocked this app's requests outright (shared hosting-platform IPs get
  // blackholed under their "no heavy automated/commercial use" policy — no header fix on
  // our side can undo that once it's happened). CARTO is a CDN-backed service explicitly
  // designed for this kind of app usage, still free, no API key/billing required.
...
async function loadPerformance() {
  ...
  try {
    ...
    if (!data.employees.length) {
      tbody.innerHTML = `<tr><td colspan="7"><div class="empty-state"><div class="icon">🏆</div>No attendance history yet to score.</div></td></tr>`;
      return;
    }
    ...
    tbody.innerHTML = `<tr><td colspan="7"><div class="empty-state"><div class="icon">⚠️</div>${escapeHtml(err.message)}</div></td></tr>`;
  }
}
...
async function loadRegularization() {
  ...
  tbody.innerHTML = `<tr><td colspan="9"><div class="empty-state"><div class="icon">⏳</div>Loading…</div></td></tr>`;
  try {
    ...
    if (!data.requests.length) {
      tbody.innerHTML = `<tr><td colspan="9"><div class="empty-state"><div class="icon">✏️</div>No correction requests.</div></td></tr>`;
      return;
    }
    tbody.innerHTML = data.requests.map(r => `
      <tr>
...
async function loadIncidents() {
  ...
  try {
    ...
    if (!data.reports.length) {
      tbody.innerHTML = `<tr><td colspan="9"><div class="empty-state"><div class="icon">🚧</div>No incident reports.</div></td></tr>`;
      return;
    }
...
async function loadReferrals() {
  ...
  try {
    ...
    if (!data.referrals.length) {
      tbody.innerHTML = `<tr><td colspan="9"><div class="empty-state"><div class="icon">👥</div>No referrals yet.</div></td></tr>`;
      return;
    }
