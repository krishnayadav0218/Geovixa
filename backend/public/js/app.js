...
function getCustomRoleLabel() { return sessionStorage.getItem('krystal_role_label') || null; }
function getEmployeeInfo() {
  try { return JSON.parse(sessionStorage.getItem('krystal_employee') || 'null'); } catch (e) { return null; }
}
function saveSession(token, role, employee, project, roleLabel) {
  ...
  if (roleLabel) sessionStorage.setItem('krystal_role_label', roleLabel);
  else sessionStorage.removeItem('krystal_role_label');
}
function clearSession() {
  ...
  sessionStorage.removeItem('krystal_role_label');
}
...
async function doStaffLogin(kind) {
  ...
  try {
    ...
    saveSession(data.token, data.role, null, data.project || null, data.custom_role_name || null);
...
document.getElementById('download-shiftcycle-excel-btn').addEventListener('click', () => {
  ...
  downloadFileWithAuth(
    ...
    'Krystal_Connect_Shift_Cycle_Report.xlsx',
...
document.getElementById('download-salary-requests-excel-btn').addEventListener('click', () => {
  ...
  downloadFileWithAuth(url, 'Krystal_Connect_Salary_Slip_Requests_Report.xlsx', 'Salary slip requests report downloaded');
});
...
document.getElementById('download-leave-requests-excel-btn').addEventListener('click', () => {
  ...
  downloadFileWithAuth(url, 'Krystal_Connect_Leave_Requests_Report.xlsx', 'Leave requests report downloaded');
});
...
async function loadProjects() {
  try {
    ...
    const selectIds = ['add-emp-project', 'edit-emp-project', 'filter-project', 'report-project', 'punch-report-project', 'employee-report-project', 'shiftcycle-report-project', 'leave-requests-project-filter', 'add-mgr-project', 'add-coordinator-project', 'edit-mgr-project', 'add-role-project', 'edit-role-account-project'];
