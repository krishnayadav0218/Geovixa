...
document.querySelectorAll('.nav-item[data-tab]').forEach(item => {
  item.addEventListener('click', () => {
    ...
    if (item.dataset.tab === 'employees') { loadProjects(); loadShiftCategories(); loadEmployees(); }
...
async function loadProjects() {
  try {
    ...
    const selectIds = ['add-emp-project', 'edit-emp-project', 'emp-filter-project', 'filter-project', 'report-project', 'employee-report-project', 'add-mgr-project', 'add-coordinator-project'];
...
document.getElementById('edit-emp-save-btn').addEventListener('click', async () => {
  ...
  try {
    await apiFetch(`/employees/${employeeId}`, {
      method: 'PUT',
      body: JSON.stringify({ name, designation, phone, location, doj, project, shift_category })
    });
