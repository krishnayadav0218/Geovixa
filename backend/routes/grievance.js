...
router.post('/my/submit', verifyEmployee, async (req, res) => {
  ...
  if (!settings.features.grievance) {
    return res.status(403).json({ error: 'Raise a Concern (Grievances) is not enabled for your company. Contact your admin.' });
  }
  ...
  await pool.query(
    `INSERT INTO grievances (employee_id, project, category, subject, description, attachment_url, status, company_id)
     VALUES ($1, $2, $3, $4, $5, $6, 'pending', $7)`,
    [emp.employee_id, emp.project || '', category, subject, description, attachmentUrl, req.user.company_id]
  );
...
router.get('/my/list', verifyEmployee, async (req, res) => {
  const { rows } = await pool.query(
    `SELECT id, category, subject, description, attachment_url, status, resolution_note,
     ...
     FROM grievances WHERE employee_id = $1 AND company_id = $2 ORDER BY requested_at DESC`,
    [req.user.employee_id, req.user.company_id]
  );
  res.json({ requests: rows });
});

// ---- admin/manager/coordinator: list grievances, scoped to their own project(s) AND own
// company — same project-scoping behaviour as Leave Requests / Salary Slip Requests ----
// GET /api/grievance/list?status=pending&category=&project=MTDC
router.get('/list', verifyAdminOrManager, async (req, res) => {
  const allowed = await checkRolePermission(pool, req.user.company_id, req.user.role, 'grievances');
  if (!allowed) return res.status(403).json({ error: 'You do not have access to the Grievances section.' });
  ...
  let query = `
    ...
    FROM grievances g
    LEFT JOIN employees e ON e.employee_id = g.employee_id AND e.company_id = g.company_id
...
// AND own company — same lock as Leave Requests / Salary Slip Requests) ----
async function reviewGrievance(req, res, newStatus) {
  const { rows } = await pool.query(
    'SELECT * FROM grievances WHERE id = $1 AND company_id = $2',
  ...
  await pool.query(
    'UPDATE grievances SET status = $1, reviewed_at = NOW(), reviewed_by = $2, resolution_note = COALESCE($3, resolution_note) WHERE id = $4 AND company_id = $5',
    [newStatus, req.user.username || req.user.name || req.user.role, note, req.params.id, req.user.company_id]
  );

  // Best-effort notification email — only for the two terminal outcomes (resolved/rejected);
  // "in_review" is just a status flip mid-process, not worth an email. Only sent if the
  // employee has an email on file AND SMTP is configured (see mailer.js).
  if (newStatus === 'resolved' || newStatus === 'rejected') {
    const empResult = await pool.query('SELECT name, email FROM employees WHERE employee_id = $1 AND company_id = $2', [grievance.employee_id, req.user.company_id]);
    const emp = empResult.rows[0];
    if (emp && emp.email) {
      sendRequestStatusEmail({
        to: emp.email, employeeName: emp.name, requestType: 'Grievance', status: newStatus,
        extra: grievance.subject,
      });
    }
  }
...
router.get('/export/excel', verifyAdminOrManager, async (req, res) => {
  ...
  const filename = `${filenameSafe(branding.name)}_Grievances_Report.xlsx`;
