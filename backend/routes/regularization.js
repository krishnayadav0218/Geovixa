...
// ---- admin/manager/coordinator: bulk-approve/reject a batch of pending requests in one go ----
// PUT /api/regularization/bulk-approve  body: { ids: [1,2,3] }
// PUT /api/regularization/bulk-reject   body: { ids: [1,2,3], note? }
// Runs each id through the exact same `review()` used by the single approve/reject buttons
// above, so bulk actions can never drift from the individual-approval rules (attendance
// record creation, notification, email) — this is purely a loop over the same logic, not a
// second copy of it. ids missing/empty a 400s: unlike overtime's bulk-approve (which defaults
// to "every pending record in scope"), regularization requests directly correct someone's
// attendance record, so silently approving every pending one company-wide is too risky to
// allow as a default — the admin must explicitly select which ones.
async function bulkReview(req, res, newStatus) {
  const ids = Array.isArray(req.body.ids) ? req.body.ids.filter((id) => Number.isInteger(Number(id))) : [];
  if (!ids.length) return res.status(400).json({ error: 'ids (array of request ids) is required' });

  const results = { approved: 0, rejected: 0, skipped: 0 };
  for (const id of ids) {
    const fakeReq = { ...req, params: { id }, body: { note: req.body.note } };
    let statusCode = 200;
    const fakeRes = { status(code) { statusCode = code; return this; }, json() {} };
    await review(fakeReq, fakeRes, newStatus);
    if (statusCode === 200) results[newStatus === 'approved' ? 'approved' : 'rejected']++;
    else results.skipped++;
  }

  await logAction(req, `regularization_bulk_${newStatus}`, {
    targetType: 'regularization_request', details: `${ids.length} request(s) — ${JSON.stringify(results)}`,
  });
  res.json({ message: `${results[newStatus === 'approved' ? 'approved' : 'rejected']} request(s) ${newStatus}, ${results.skipped} skipped`, ...results });
}
router.put('/bulk-approve', verifyAdminOrManager, (req, res) => bulkReview(req, res, 'approved'));
router.put('/bulk-reject', verifyAdminOrManager, (req, res) => bulkReview(req, res, 'rejected'));
