...
// ---------------------------------------------------------------------------------------
// POST /api/reliever/rank  body: { project, lat?, lng?, weekly_ot_ceiling_hours? }
router.post('/rank', verifyAdminOrManager, async (req, res) => {
  ...
  const weeklyOtCeiling = Number(req.body.weekly_ot_ceiling_hours) || 20; // typical statutory-ish weekly OT cap, admin-overridable
  ...
  if (!employees.length) return res.json({ project, ranked: [] });
  const empIds = employees.map(e => e.employee_id);
  ...
  // Last-known GPS (for distance scoring) + last 30 days attendance % (for reliability) +
  // last 30 days OT hours (for OT-eligibility) — all pulled in bulk, one query each, rather
  // than N+1 per employee.
  ...
  const ranked = candidates.map(e => {
    const loc = lastLoc.get(e.employee_id);
  ...
  res.json({ project, candidateCount: ranked.length, ranked: ranked.slice(0, 20) });
});
