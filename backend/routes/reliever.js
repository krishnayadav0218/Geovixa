...
router.post('/assign', verifyAdminOrManager, async (req, res) => {
  ...
  // site_location_id: assigning to cover a general SHORTAGE at a specific sub-location
  // (Site Locations) rather than one named absent person — in that case
  // original_employee_id isn't required, mirroring how Auto-Assign covers shortages.
  const site_location_id = req.body.site_location_id ? Number(req.body.site_location_id) : null;
  ...
  if (!reliever_employee_id || !isValidDate(duty_date)) {
    return res.status(400).json({ error: 'reliever_employee_id and duty_date (YYYY-MM-DD) are required' });
  }
  if (!original_employee_id && !site_location_id) {
    return res.status(400).json({ error: 'Either original_employee_id (covering a specific person) or site_location_id (covering a location shortage) is required' });
  }
  ...
  const idsToCheck = [reliever_employee_id];
  if (original_employee_id) idsToCheck.push(original_employee_id);
  const { rows } = await pool.query(
    `SELECT employee_id, name, project, active FROM employees WHERE company_id = $1 AND employee_id = ANY($2::text[])`,
    [companyId, idsToCheck]
  );
  const original = original_employee_id ? rows.find(r => r.employee_id === original_employee_id) : null;
  const reliever = rows.find(r => r.employee_id === reliever_employee_id);
  if (original_employee_id && !original) return res.status(404).json({ error: 'Original employee not found' });
  ...
  let project = original ? original.project : (req.body.project || '').trim();
  let locationName = null;
  if (site_location_id) {
    const { rows: locRows } = await pool.query('SELECT project, name FROM site_locations WHERE id = $1 AND company_id = $2', [site_location_id, companyId]);
    if (!locRows[0]) return res.status(404).json({ error: 'Location not found' });
    project = locRows[0].project;
    locationName = locRows[0].name;
  }
  if (!project) return res.status(400).json({ error: 'Could not determine which project this assignment belongs to' });
  ...
  if (projects && projects.length && !projects.includes(project)) {
    return res.status(403).json({ error: 'This is not in your project' });
  }
  ...
  const { rows: inserted } = await pool.query(
    `INSERT INTO reliever_assignments
     ...
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, ${force ? 'NOW()' : 'NULL'})
     RETURNING id`,
    [companyId, original_employee_id || null, reliever_employee_id, project || '', site_location_id, duty_date, reason || null,
  ...
  await logAction(req, force ? 'reliever_force_assigned' : 'reliever_assigned', {
    ...
    targetLabel: `${reliever_employee_id} covering ${original_employee_id || (locationName ? `${project} — ${locationName} shortage` : project)} on ${duty_date}${force ? ' (forced)' : ''}`,
  });

  await notifyEmployee(
    companyId, reliever_employee_id, 'reliever_assigned',
    force ? 'You have been placed on reliever duty' : 'New reliever duty assigned',
    `${project || locationName || 'A site'} on ${duty_date}${force ? '' : ' — please accept or reject.'}`
  );
...
async function rankRelieversForSite(companyId, project, { lat: fallbackLat, lng: fallbackLng, weeklyOtCeilingHours, limit, siteLocationId } = {}) {
  ...
  // If a specific SUB-LOCATION is targeted (one of possibly many under this project — see
  // Site Locations), its own GPS is the reference point, not the project's. This is what
  // makes ranking correctly nearest-to-the-actual-shortage-spot rather than nearest-to-the-
  // project's-single-average-point when a project spans many physical locations.
  let targetLocation = null;
  if (siteLocationId) {
    const { rows: locRows } = await pool.query('SELECT * FROM site_locations WHERE id = $1 AND company_id = $2', [siteLocationId, companyId]);
    targetLocation = locRows[0] || null;
  }

  // The reference point for "nearby" is the SHORTAGE SITE's (or sub-location's) own saved
  // location — not whoever happens to be running this from their browser. A dispatcher
  // opening this from home shouldn't skew every ranking towards their own house. lat/lng
  // args are kept only as a fallback for the rare case nothing has GPS saved yet.
  const lat = targetLocation ? Number(targetLocation.latitude)
    : site && site.latitude != null ? Number(site.latitude) : (fallbackLat != null ? Number(fallbackLat) : null);
  const lng = targetLocation ? Number(targetLocation.longitude)
    : site && site.longitude != null ? Number(site.longitude) : (fallbackLng != null ? Number(fallbackLng) : null);
  if (lat == null || lng == null) {
    const label = targetLocation ? `${project} — ${targetLocation.name}` : project;
    const err = new Error(`"${label}" has no GPS location saved yet. Add it under Operations Map → Edit Site Details / Manage Locations, so ranking knows what "nearby" means here.`);
    err.status = 400;
    throw err;
  }
  ...
  if (!employees.length) return { project, site, siteLocation: targetLocation, candidateCount: 0, ranked: [] };
  const empIds = employees.map(e => e.employee_id);
  ...
  return { project, site, siteLocation: targetLocation, candidateCount: ranked.length, ranking_method: usingMl ? 'ml' : 'heuristic', ranked: ranked.slice(0, Math.min(limit || 20, 50)) };
}
...
router.post('/rank', verifyAdminOrManager, async (req, res) => {
  ...
  try {
    const result = await rankRelieversForSite(req.user.company_id, project, {
      lat: req.body.lat, lng: req.body.lng, weeklyOtCeilingHours: req.body.weekly_ot_ceiling_hours,
      limit: req.body.limit, siteLocationId: req.body.site_location_id,
    });
    res.json({
      project: result.project, siteLocation: result.siteLocation,
      candidateCount: result.candidateCount, ranking_method: result.ranking_method, ranked: result.ranked,
    });
  } catch (err) {
