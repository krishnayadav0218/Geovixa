...
async function rankRelieversForSite(companyId, project, { lat: fallbackLat, lng: fallbackLng, weeklyOtCeilingHours, limit } = {}) {
  ...
  const ranked = candidates.map(e => {
    ...
    return {
      ...
      is_live_location: liveLoc.has(e.employee_id),
  ...
  return { project, site, candidateCount: ranked.length, ranked: ranked.slice(0, Math.min(limit || 20, 50)) };
}
...
router.post('/rank', verifyAdminOrManager, async (req, res) => {
  ...
  try {
    ...
    res.json({ project: result.project, candidateCount: result.candidateCount, ranked: result.ranked });
  } catch (err) {
