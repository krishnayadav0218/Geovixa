...
// GET /api/site-locations/mine — employee-scoped, read-only: just enough for the
// mobile app to draw its own assigned geofence and pre-check "am I close enough"
// before capturing a selfie. Deliberately does NOT expose other employees' locations
// or full company site data — that stays admin/manager-only via GET /.
router.get('/mine', verifyEmployee, async (req, res) => {
  ...
  if (!empRows[0]) return res.json({ locations: [] });
  const { project, site_location_id } = empRows[0];

  if (site_location_id) {
    const { rows } = await pool.query(
      'SELECT id, name, latitude, longitude, radius_m, project FROM site_locations WHERE id = $1 AND company_id = $2',
      [site_location_id, req.user.company_id]
    );
    if (rows[0]) return res.json({ locations: rows });
  }
  if (project) {
    const { rows } = await pool.query(
      'SELECT id, name, latitude, longitude, radius_m, project FROM site_locations WHERE project = $1 AND company_id = $2 ORDER BY name',
      [project, req.user.company_id]
    );
    if (rows.length) return res.json({ locations: rows });

    // No sub-locations defined for this project — fall back to the project's own
    // geofence (latitude/longitude/geofence_radius_m), same one attendance/punch
    // already validates punches against server-side.
    const { rows: projRows } = await pool.query(
      'SELECT latitude, longitude, geofence_radius_m FROM projects WHERE name = $1 AND company_id = $2',
      [project, req.user.company_id]
    );
    if (projRows[0] && projRows[0].latitude != null && projRows[0].longitude != null) {
      return res.json({
        locations: [{
          id: 0, name: project, project,
          latitude: projRows[0].latitude, longitude: projRows[0].longitude,
          radius_m: projRows[0].geofence_radius_m,
        }],
      });
    }
  }
