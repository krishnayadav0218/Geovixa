...
router.post('/', verifyEmployee, async (req, res) => {
  ...
  // Real-time push — every connected Admin/Manager/Coordinator for this company gets this
  // instantly instead of waiting for the next 15s SOS-panel poll (see app.js loadSosAlerts).
  emitToCompany(req.user.company_id, 'sos:new', {
    id: rows[0].id, employee_id: req.user.employee_id, project, type,
...
router.put('/:id/acknowledge', verifyAdminOrManager, async (req, res) => {
  ...
  emitToCompany(req.user.company_id, 'sos:updated', { id: Number(req.params.id), status: 'acknowledged' });
...
router.put('/:id/resolve', verifyAdminOrManager, async (req, res) => {
  ...
  emitToCompany(req.user.company_id, 'sos:updated', { id: Number(req.params.id), status: 'resolved' });
