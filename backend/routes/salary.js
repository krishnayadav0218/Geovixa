...
async function buildSalarySlip(employee, month) {
  ...
  const sal = salResult.rows[0] || { basic_salary: 0, hra: 0, other_allowances: 0, deductions: 0 };
  ...
  const deductions = round2(sal.deductions);
  ...
  const netPay = round2(earnedGross - deductions);
  ...
  return {
    ...
    salaryStructure: { basic, hra, otherAllowances, deductions },
...
function streamSlipPdf(res, slip) {
  ...
  doc.text(slip.deductions.toFixed(2), col2, y, { width: colWidth, align: 'right' });
  doc.moveDown(0.6);
...
router.get('/:employeeId', verifyAdminOrManager, async (req, res) => {
  ...
  const sal = rows[0] || { employee_id: req.params.employeeId.trim(), basic_salary: 0, hra: 0, other_allowances: 0, deductions: 0 };
  res.json({ salary: sal });
});
...
router.put('/:employeeId', verifyAdmin, async (req, res) => {
  ...
  const { basic_salary, hra, other_allowances, deductions } = req.body;
  ...
  await pool.query(
    `INSERT INTO salaries (employee_id, basic_salary, hra, other_allowances, deductions, updated_at)
     ...
     ON CONFLICT (employee_id) DO UPDATE SET
       ...
       updated_at = NOW()`,
    [employeeId, Number(basic_salary) || 0, Number(hra) || 0, Number(other_allowances) || 0, Number(deductions) || 0]
  );
