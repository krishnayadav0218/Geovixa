...
const { resolveLogoPath } = require('../logoStorage');
...
// `companyId` scopes every lookup to one tenant (two companies can each have their own
// salaries row / attendance / shift categories / weekly-off day for the same employee_id).
async function buildSalarySlip(employee, month, companyId) {
  const { daysInMonth, from, to } = monthRange(month);

  const [salResult, attResult, thresholdsMap, weeklyOffMap, companyResult] = await Promise.all([
    pool.query('SELECT * FROM salaries WHERE employee_id = $1 AND company_id = $2', [employee.employee_id, companyId]),
    pool.query(
      ...
      [employee.employee_id, companyId, from, to]
    ),
    loadShiftThresholdsMap(pool, companyId),
    loadWeeklyOffMap(pool, companyId),
    pool.query('SELECT name, logo_url FROM companies WHERE id = $1', [companyId]),
  ]);

  const company = companyResult.rows[0] || { name: 'Geovixa', logo_url: null };
  ...
  return {
    ...
    month,
    daysInMonth,
    companyName: company.name || 'Geovixa',
...
function streamSlipPdf(res, slip) {
  ...
  // ---- header ----
  // If this company has uploaded a logo (super_admin -> Companies -> Logo), print it at the
  // top-left of the slip and shift the company name/heading text to the right of it. If
  // there's no logo, or the file's gone missing from disk for some reason, falls back to the
  // plain text-only header exactly like before — never lets a missing logo break the PDF.
  const logoPath = resolveLogoPath(slip.companyLogoUrl);
  const companyName = slip.companyName || 'Geovixa';
  let textStartX = 50;
  if (logoPath) {
    try {
      doc.image(logoPath, 50, doc.y, { fit: [55, 55] });
      textStartX = 115;
    } catch (err) {
      textStartX = 50; // corrupt/unreadable image file — just fall back to text-only header
    }
  }
  const headerTop = doc.y;
  doc.fillColor('#071A2C').fontSize(20).font('Helvetica-Bold').text(companyName, textStartX, headerTop, { align: 'left' });
  doc.fontSize(11).font('Helvetica').fillColor('#64748B').text('Salary Slip', textStartX, doc.y, { align: 'left' });
  doc.moveDown(0.3);
  doc.fillColor('#0B93D6').fontSize(12).font('Helvetica-Bold')
    .text(`For the month of ${slip.month}`, textStartX, doc.y, { align: 'left' });
  if (logoPath) {
    // Make sure the divider line below doesn't get drawn through the logo image if the logo
    // (55pt tall) is taller than the 3 lines of header text next to it.
    doc.y = Math.max(doc.y, headerTop + 55 + 8);
  }
  doc.x = 50; // reset the cursor back to the left margin for everything below the header
  doc.moveDown(1);
...
router.get('/my/slip-requests', verifyEmployee, async (req, res) => {
  const { rows } = await pool.query(
    'SELECT id, month, status, requested_at, reviewed_at FROM salary_slip_requests WHERE employee_id = $1 AND company_id = $2 ORDER BY month DESC',
...
router.post('/my/slip-request', verifyEmployee, async (req, res) => {
  ...
  if (!settings.features.salary) {
    return res.status(403).json({ error: 'Salary Slips is not enabled for your company. Contact your admin.' });
  }
  ...
  try {
    await pool.query(
      `INSERT INTO salary_slip_requests (employee_id, month, project, status, company_id)
       VALUES ($1, $2, $3, 'pending', $4)`,
      [emp.employee_id, month, emp.project || '', req.user.company_id]
    );
...
// employee+month (within one company) and turns its status into a clear message when it
// isn't approved yet.
async function requireApprovedRequest(employeeId, month, companyId) {
  const { rows } = await pool.query(
    'SELECT status FROM salary_slip_requests WHERE employee_id = $1 AND month = $2 AND company_id = $3',
    [employeeId, month, companyId]
  );
...
router.get('/my/slip', verifyEmployee, async (req, res) => {
  ...
  const approval = await requireApprovedRequest(req.user.employee_id, month, req.user.company_id);
  ...
  const slip = await buildSalarySlip(emp, month, req.user.company_id);
  res.json({ slip });
});
...
// AND own company (admin sees every project unless ?project=/?status= is passed to narrow it down) ----
// GET /api/salary/requests?status=pending
router.get('/requests', verifyAdminOrManager, async (req, res) => {
  const allowed = await checkRolePermission(pool, req.user.company_id, req.user.role, 'salary_requests');
  if (!allowed) return res.status(403).json({ error: 'You do not have access to the Salary Slip Requests section.' });
...
// ---- admin/manager/coordinator: approve/reject a request (only within their own project
// scope AND own company) ----
async function reviewRequest(req, res, newStatus) {
  const { rows } = await pool.query(
    'SELECT * FROM salary_slip_requests WHERE id = $1 AND company_id = $2',
  ...
  await pool.query(
    'UPDATE salary_slip_requests SET status = $1, reviewed_at = NOW(), reviewed_by = $2 WHERE id = $3 AND company_id = $4',
    [newStatus, req.user.username || req.user.name || req.user.role, req.params.id, req.user.company_id]
  );

  // Best-effort notification email (only if the employee has an email on file AND SMTP is
  // configured — see mailer.js). Never blocks or fails the actual approval/rejection.
  ...
  if (emp && emp.email) {
    sendRequestStatusEmail({
      to: emp.email, employeeName: emp.name, requestType: 'Salary Slip Request', status: newStatus,
      extra: `Month: ${request.month}`,
    });
  }
...
// ---- admin/manager: view an employee's fixed monthly salary structure (own company) ----
router.get('/:employeeId', verifyAdminOrManager, async (req, res) => {
  const { rows } = await pool.query(
    'SELECT * FROM salaries WHERE employee_id = $1 AND company_id = $2',
    [req.params.employeeId.trim(), req.user.company_id]
  );
  const sal = rows[0] || { employee_id: req.params.employeeId.trim(), basic_salary: 0, hra: 0, other_allowances: 0, deductions: 0, pf: 0, esic: 0 };
  res.json({ salary: sal });
});

// ---- admin only: set/update an employee's fixed monthly salary structure (own company) ----
router.put('/:employeeId', verifyAdmin, async (req, res) => {
  ...
  const empResult = await pool.query('SELECT employee_id FROM employees WHERE employee_id = $1 AND company_id = $2', [employeeId, companyId]);
  ...
  await pool.query(
    `INSERT INTO salaries (employee_id, basic_salary, hra, other_allowances, deductions, pf, esic, updated_at, company_id)
     ...
     ON CONFLICT (company_id, employee_id) DO UPDATE SET
       ...
       esic = EXCLUDED.esic,
       updated_at = NOW()`,
    [employeeId, Number(basic_salary) || 0, Number(hra) || 0, Number(other_allowances) || 0, Number(deductions) || 0, Number(pf) || 0, Number(esic) || 0, companyId]
  );
  res.json({ message: 'Salary structure updated' });
});

// ---- admin/manager: view any employee's computed salary slip for a month (own company) ----
...
// ---- admin/manager: download any employee's salary slip PDF (own company) ----
