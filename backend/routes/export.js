...
router.get('/excel', async (req, res) => {
  ...
  const weeklyOffMap = await loadWeeklyOffMap(pool);
  ...
  const leaveMap = await loadApprovedLeaveMap(pool, from || '0001-01-01', to || todayDateStr());
...
router.get('/summary-excel', async (req, res) => {
  ...
  let empQuery = 'SELECT employee_id, name, designation, location, doj, project, shift_category, zone, ward, site_code FROM employees WHERE active = 1';
  const empParams = [];
...
router.get('/employees-excel', async (req, res) => {
  ...
  let query = 'SELECT employee_id, name, designation, phone, location, doj, project, shift_category, zone, ward, site_code, active, created_at FROM employees WHERE 1=1';
  const params = [];
...
router.get('/shift-cycle-excel', verifyAdminOrManager, async (req, res) => {
  ...
  let empQuery = `
    ...
    FROM employees
    WHERE active = 1 AND shift_category = '8 Hrs - FA'`;
  const empParams = [];
  if (location) { empParams.push(location); empQuery += ` AND location = ANY($${empParams.length}::text[])`; }
  if (zone) { empParams.push(zone); empQuery += ` AND zone = ANY($${empParams.length}::text[])`; }
  if (ward) { empParams.push(ward); empQuery += ` AND ward = ANY($${empParams.length}::text[])`; }
  ...
  const attResult = empIds.length
    ? await pool.query(
        `SELECT employee_id, status, latitude, longitude, address, server_time, attendance_date
         FROM attendance
         WHERE employee_id = ANY($1::text[]) AND attendance_date >= $2 AND attendance_date <= $3
         ORDER BY employee_id ASC, server_time ASC`,
        [empIds, fromDate, fetchToStr]
      )
    : { rows: [] };
