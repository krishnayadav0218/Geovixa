...
router.get('/excel', async (req, res) => {
  const { from, to, employee_id, location, zone, ward } = req.query;
  ...
  if (zone) { params.push(zone); query += ` AND e.zone = $${params.length}`; }
  if (ward) { params.push(ward); query += ` AND e.ward = $${params.length}`; }
...
router.get('/summary-excel', async (req, res) => {
  ...
  const { location, zone, ward } = req.query;
  const projects = await effectiveProjects(req, pool);
  ...
  if (zone) { empParams.push(zone); empQuery += ` AND zone = $${empParams.length}`; }
  if (ward) { empParams.push(ward); empQuery += ` AND ward = $${empParams.length}`; }
