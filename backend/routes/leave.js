...
// ---- employee (self) — leave balance for the current calendar year ----
// GET /api/leave/my/balance
router.get('/my/balance', verifyEmployee, async (req, res) => {
  const year = new Date().getFullYear();
  const { rows: balRows } = await pool.query(
    'SELECT allocated_days FROM leave_balances WHERE company_id = $1 AND employee_id = $2 AND year = $3',
    [req.user.company_id, req.user.employee_id, year]
  );
  // No row yet = never explicitly allocated — falls back to a sane default (12/year)
  // rather than showing 0, which would look like the employee has no leave at all.
  const allocatedDays = balRows[0] ? Number(balRows[0].allocated_days) : 12;
  ...
  const { rows: approvedRows } = await pool.query(
    `SELECT from_date, to_date FROM leave_requests
     WHERE employee_id = $1 AND company_id = $2 AND status = 'approved'
       AND EXTRACT(YEAR FROM from_date::date) = $3`,
    [req.user.employee_id, req.user.company_id, year]
  );
  const usedDays = approvedRows.reduce((sum, r) => sum + daysInclusive(r.from_date, r.to_date), 0);
  ...
  const pendingDays = pendingRows.reduce((sum, r) => sum + daysInclusive(r.from_date, r.to_date), 0);

  res.json({
    year, allocated_days: allocatedDays, used_days: usedDays, pending_days: pendingDays,
    remaining_days: Math.max(0, allocatedDays - usedDays),
  });
});

// ---- admin/manager: set an employee's leave allocation for a year ----
// PUT /api/leave/balance/:employeeId  body: { year?, allocated_days }
router.put('/balance/:employeeId', verifyAdminOrManager, async (req, res) => {
  const year = Number(req.body.year) || new Date().getFullYear();
  const allocatedDays = Number(req.body.allocated_days);
  if (!Number.isFinite(allocatedDays) || allocatedDays < 0) {
    return res.status(400).json({ error: 'allocated_days must be a non-negative number' });
  }
  await pool.query(
    `INSERT INTO leave_balances (company_id, employee_id, year, allocated_days)
     VALUES ($1, $2, $3, $4)
     ON CONFLICT (company_id, employee_id, year)
     DO UPDATE SET allocated_days = $4, updated_at = NOW()`,
    [req.user.company_id, req.params.employeeId, year, allocatedDays]
  );
