...
router.get('/coordinator-accounts', verifyAdmin, async (req, res) => {
  const { rows } = await pool.query(
    "SELECT id, username, created_at FROM admins WHERE role = 'coordinator' ORDER BY id DESC"
  );
...
router.post('/coordinator-accounts', verifyAdmin, async (req, res) => {
  ...
  try {
    const hash = bcrypt.hashSync(password, 10);
    await pool.query(
      "INSERT INTO admins (username, password_hash, role) VALUES ($1, $2, 'coordinator')",
      [username.trim(), hash]
    );
