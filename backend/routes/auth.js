...
// Looks up an ACTIVE company by its Company Code (case-insensitive). Every non-super-admin
// login (Admin/Manager/Coordinator/report-only-role AND Employee) now starts here — the
// Company Code is how the shared app knows which tenant's data to check against.
...
router.post('/employee-login', async (req, res) => {
  ...
  if (!emp.pin_hash) {
    return res.status(403).json({ error: 'No PIN has been set for this Employee ID yet. Contact admin to set one.' });
  }
