...
const { logLoginAttempt } = require('../auditLog');
...
async function handleLogin(req, res) {
  ...
  if (!companyResult.ok) {
    await logLoginAttempt(req, { username, success: false, reason: companyResult.error });
  ...
  if (!account) {
    await logLoginAttempt(req, { companyId: company.id, username, success: false, reason: 'Unknown username' });
  ...
  if (!valid) {
    await logLoginAttempt(req, { companyId: company.id, username, role: account.role, success: false, reason: 'Wrong password' });
    return res.status(401).json({ error: 'Invalid Company Code, username or password' });
  }
  ...
  await logLoginAttempt(req, { companyId: company.id, username: account.username, role: account.role, success: true });
...
router.post('/employee-login', async (req, res) => {
  ...
  const emp = rows[0];
  if (!emp) {
    await logLoginAttempt(req, { companyId: company.id, username: employee_id, role: 'employee', success: false, reason: 'Unknown employee ID' });
    return res.status(401).json({ error: 'Incorrect Company Code or Employee ID. Please check and try again.' });
  }
  if (!emp.active) {
    await logLoginAttempt(req, { companyId: company.id, username: employee_id, role: 'employee', success: false, reason: 'Deactivated employee' });
    return res.status(403).json({ error: 'This Employee ID is deactivated. Contact admin.' });
  }
  ...
  if (emp.pin_hash) {
    ...
    if (!pinValid) {
      await logLoginAttempt(req, { companyId: company.id, username: employee_id, role: 'employee', success: false, reason: 'Wrong PIN' });
  ...
  const token = signToken(payload);
  const settings = await getCompanySettings(pool, company.id);
  await logLoginAttempt(req, { companyId: company.id, username: emp.employee_id, role: 'employee', success: true });
...
// ---- Client Portal login — a fundamentally different, read-only account type (see
// routes/clientAccounts.js for how admins create these, routes/clientPortal.js for what a
// logged-in client can actually see). ----
router.post('/client-login', async (req, res) => {
  ...
  const companyResult = await findActiveCompany(company_code);
  if (!companyResult.ok) {
    await logLoginAttempt(req, { username, role: 'client', success: false, reason: companyResult.error });
    return res.status(400).json({ error: companyResult.error });
  }
  const company = companyResult.company;

  const { rows } = await pool.query('SELECT * FROM clients WHERE company_id = $1 AND username = $2', [company.id, username.trim()]);
  const account = rows[0];
  if (!account || !account.active) {
    await logLoginAttempt(req, { companyId: company.id, username, role: 'client', success: false, reason: !account ? 'Unknown username' : 'Deactivated account' });
    return res.status(401).json({ error: 'Invalid Company Code, username or password' });
  }
  const valid = bcrypt.compareSync(password, account.password_hash);
  if (!valid) {
    await logLoginAttempt(req, { companyId: company.id, username, role: 'client', success: false, reason: 'Wrong password' });
    return res.status(401).json({ error: 'Invalid Company Code, username or password' });
  }

  const { rows: siteRows } = await pool.query('SELECT project FROM client_sites WHERE client_id = $1', [account.id]);
  const payload = {
    id: account.id, username: account.username, name: account.name, role: 'client',
    projects: siteRows.map(r => r.project),
    company_id: company.id, company_name: company.name, company_code: company.code,
  };
  const token = signToken(payload);
  await logLoginAttempt(req, { companyId: company.id, username: account.username, role: 'client', success: true });
