...
// POST /api/auth/employee-login  -> employee logs in with Employee ID + PIN
// SECURITY: Employee ID alone is not a secret (printed on ID cards, visible in reports),
// so login now always requires the PIN the admin set for that employee too.
router.post('/employee-login', async (req, res) => {
  ...
  if (!emp.pin_hash) {
    // Should only happen for a brand-new employee added through an older client build.
    return res.status(403).json({ error: 'No PIN has been set for this Employee ID yet. Ask your admin to set one.' });
  }

  const validPin = bcrypt.compareSync(String(pin).trim(), emp.pin_hash);
  if (!validPin) {
