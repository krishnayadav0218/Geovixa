...
router.post('/', (req, res) => {
  ...
  try {
    db.prepare('INSERT INTO employees (employee_id, name, designation, phone, location, doj) VALUES (?, ?, ?, ?, ?, ?)')
      .run(employee_id.trim(), name.trim(), designation || '', phone || '', location || '', doj || '');
...
router.post('/bulk', (req, res) => {
  ...
  const insert = db.prepare('INSERT OR IGNORE INTO employees (employee_id, name, designation, phone, location, doj) VALUES (?, ?, ?, ?, ?, ?)');
  const insertMany = db.transaction((rows) => {
    let added = 0;
    for (const e of rows) {
      if (e.employee_id && e.name) {
        const info = insert.run(String(e.employee_id).trim(), String(e.name).trim(), e.designation || '', e.phone || '', e.location || '', e.doj || '');
...
router.put('/:employeeId', (req, res) => {
  ...
  db.prepare('UPDATE employees SET name = ?, designation = ?, phone = ?, location = ?, doj = ?, active = ? WHERE employee_id = ?')
    .run(
      name ?? existing.name,
