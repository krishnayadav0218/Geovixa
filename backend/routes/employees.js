...
router.post('/', async (req, res) => {
  ...
  // Employee login is by Employee ID alone now, so PIN is optional (kept only for
  // backward compatibility with older client builds / bulk-imported data).
  let pinHash = null;
  if (pin !== undefined && pin !== null && String(pin).trim() !== '') {
    ...
    pinHash = bcrypt.hashSync(String(pin).trim(), 10);
  }
