...
router.post('/punch', optionalAuth, async (req, res) => {
  ...
  // Geofence check — if this employee's site has a GPS location + radius saved (Site
  // Management), both punch-in AND punch-out must happen from within that radius. A site
  // with no location saved yet has no geofence to enforce (opt-in per site, not global).
  if (emp.project) {
    ...
    if (site && site.latitude != null && site.longitude != null) {
      const distanceM = haversineKm(lat, lng, Number(site.latitude), Number(site.longitude)) * 1000;
      const radiusM = Number(site.geofence_radius_m) || 200;
      if (distanceM > radiusM) {
        return res.status(403).json({
          error: `You are ${Math.round(distanceM)}m away from ${emp.project} — you must be within ${radiusM}m of the site to ${status === 'on_duty' ? 'punch in' : 'punch out'}.`,
        });
      }
    }
  }
