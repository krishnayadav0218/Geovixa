...
router.post('/punch', optionalAuth, (req, res) => {
  ...
  // ---- Enforce: only ONE Punch In and ONE Punch Out per employee per day ----
  // Prevents duplicate/spam punches, keeps storage lean, and keeps reports accurate.
  const todaysRecords = db.prepare(
    'SELECT status FROM attendance WHERE employee_id = ? AND attendance_date = ?'
  ).all(employee_id.trim(), attendance_date);
...
// Date-wise Present/Absent matrix used by the Reports tab (admin + manager).
// "P" = at least one punch (on_duty or off_duty) recorded that day, "A" = none.
router.get('/grid', verifyAdminOrManager, (req, res) => {
  const to = req.query.to || todayDateStr();
  const from = req.query.from || to; // default to a single day if not given
  ...
  // Build a Set of "employee_id|date" that have at least one punch, for O(1) lookups.
  const presentSet = new Set(rows.map(r => `${r.employee_id}|${r.attendance_date}`));

  // Build the list of dates between from and to (inclusive).
  const dates = [];
  ...
  const grid = employees.map(emp => {
    ...
    dates.forEach(d => {
      if (emp.doj && d < emp.doj) {
        days[d] = '-'; // not yet joined — not a real absence
      } else {
