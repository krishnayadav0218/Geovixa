...
const siteLocationsRoutes = require('./routes/siteLocations');
...
//   render base64 data URIs, PLUS the OpenStreetMap tile servers (a/b/c.tile.openstreetmap.org)
//   that the Employee Tracking live map's Leaflet tile layer loads as plain <img> requests —
//   without this, the map container and library load fine but every tile image is silently
//   blocked by CSP, so the map area just stays grey/blank with no visible error to the user.
...
app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      ...
      // connectSrc governs fetch()/XHR calls (separate from imgSrc, which only covers
      // <img> tags). The Employee Tracking route-search and My Route features both call
      // Nominatim directly via fetch() to reverse-geocode a clicked point's address —
      // without this, those specific lookups fail with a CSP violation even though the
      // base map tiles (covered by imgSrc above) load fine.
  ...
  // Helmet defaults Referrer-Policy to "no-referrer", which strips the Referer header from
  // EVERY outgoing request the browser makes — including the <img> tile requests Leaflet
  // sends to OpenStreetMap's tile servers. OSM's tile usage policy requires a Referer header
  // to identify which site is using their free, volunteer-run tiles; with no-referrer set,
  // every tile request came back as a 403 "Access blocked — Referer is required" instead of
  // an actual map image. "strict-origin-when-cross-origin" sends the full URL on same-origin
  // requests but only the origin (scheme+host, no path) cross-origin — enough to satisfy OSM
  // without leaking this app's internal page paths to a third party.
...
// GET /employees/verify/:employeeId (the Android login screen's ID pre-check) is public
// and unauthenticated by design — but with no throttling, it was a straightforward
// enumeration endpoint: script through sequential Employee IDs and harvest every real
// employee's full name for a given company, with zero rate limit. Same limiter/reasoning
// as the company-lookup endpoint above.
app.use('/api/employees/verify', companyLookupLimiter);
...
app.use('/api/site-locations', siteLocationsRoutes);
...
async function start() {
  ...
  // Wrapping Express in a plain http.Server (instead of app.listen() directly) so Socket.io
  // can attach to the exact same port — Render's free tier only exposes one port per service,
  // so real-time updates (SOS, live tracking) have to share it with the REST API rather than
  // running on a second port.
  ...
  server.listen(PORT, () => {
    ...
    console.log(`   Real-time (Socket.io): enabled on the same port\n`);
  });

  // Reliever Auto-Assign — runs independently of any dashboard being open, every 5 minutes,
  // for every company that has flipped the toggle on (routes/reliever.js /auto-assign-settings).
  startAutoAssignBackgroundLoop();
  startSosEscalationLoop();
