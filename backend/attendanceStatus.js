...
// isOnApprovedLeave: true if this employee has an APPROVED leave_requests row covering this
// date (see loadApprovedLeaveMap below). Takes priority over the punch-based P/HD/A
// calculation — an approved leave day should read 'L' in every report even if the employee
// also happened to punch in/out that day.
// Returns one of: 'P' | 'HD' | 'A' | 'W/O' | 'L' | '-'
function computeDayStatus({ onDutyTime, offDutyTime, shiftCategory, isWeeklyOff, joined, thresholdsMap, isOnApprovedLeave }) {
  if (joined === false) return '-'; // before Date of Joining
  if (isOnApprovedLeave) return 'L'; // approved leave overrides punch-based P/HD/A/A(no-punch)
...
// scoped to one company (two different companies can each have their own "8 Hrs - FA"
// category with different hours). Call once per request and reuse it for every
// employee/date in that request.
async function loadShiftThresholdsMap(pool, companyId) {
  const { rows } = await pool.query('SELECT name, full_hours, half_hours FROM shift_categories WHERE company_id = $1', [companyId]);
  const map = {};
  rows.forEach(r => {
...
// scoped to one company. Call once per request. Each project can have a different
// weekly-off day (not every workforce is off on a Sunday) — see the weekly_off_day column
// on the projects table.
async function loadWeeklyOffMap(pool, companyId) {
  const { rows } = await pool.query('SELECT name, weekly_off_day FROM projects WHERE company_id = $1', [companyId]);
  const map = {};
  rows.forEach(r => {
...
// Loads every APPROVED leave_requests row (within one company) that overlaps [fromDate,
// toDate] and expands each row's [from_date, to_date] span into individual calendar days,
// returning a Set of "employee_id|YYYY-MM-DD" keys. Call once per request (grid/report) and
// reuse it for every employee/date being rendered, same pattern as
// loadShiftThresholdsMap/loadWeeklyOffMap above.
async function loadApprovedLeaveMap(pool, fromDate, toDate, companyId) {
  const { rows } = await pool.query(
    `SELECT employee_id, from_date, to_date FROM leave_requests
     WHERE status = 'approved' AND from_date <= $2 AND to_date >= $1 AND company_id = $3`,
    [fromDate, toDate, companyId]
  );
  const set = new Set();
  rows.forEach(r => {
    const cur = new Date(r.from_date + 'T00:00:00Z');
    const end = new Date(r.to_date + 'T00:00:00Z');
    while (cur <= end) {
      set.add(`${r.employee_id}|${cur.toISOString().slice(0, 10)}`);
...
module.exports = {
  ...
  loadApprovedLeaveMap, DEFAULT_THRESHOLDS, classifyShiftByLoginTime, istMinutesOfDay,
};
