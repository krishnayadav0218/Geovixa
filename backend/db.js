...
async function init() {
  ...
  // Leave balance — how many days an employee is allocated per calendar year. Used days
  // are computed on the fly from approved leave_requests rather than stored separately,
  // so there's no risk of the two drifting out of sync with each other.
  await pool.query(`
    CREATE TABLE IF NOT EXISTS leave_balances (
      ...
      allocated_days NUMERIC NOT NULL DEFAULT 12,
      updated_at TIMESTAMPTZ DEFAULT NOW(),
      UNIQUE(company_id, employee_id, year)
    );
  `);
  await pool.query('CREATE INDEX IF NOT EXISTS idx_leave_balances_employee ON leave_balances(company_id, employee_id, year);');
  ...
  // Escalation — if an open SOS sits unacknowledged past a threshold, sosEscalation.js
  // flags it here so the admin panel can visually surface it as urgent instead of it
  // silently aging in the same "open" list as everything else.
  await pool.query('ALTER TABLE sos_alerts ADD COLUMN IF NOT EXISTS escalated BOOLEAN NOT NULL DEFAULT FALSE;');
  await pool.query('ALTER TABLE sos_alerts ADD COLUMN IF NOT EXISTS escalated_at TIMESTAMPTZ;');
  ...
  // GPS anomaly flags — real impossible-travel-speed detection (see gpsAnomalyDetection.js).
  // A previous comment on this file claimed punches like this were "blocked outright",
  // but no such check actually existed anywhere in the codebase until now.
  await pool.query(`
    CREATE TABLE IF NOT EXISTS gps_anomaly_flags (
      id SERIAL PRIMARY KEY,
      company_id INTEGER REFERENCES companies(id),
      employee_id TEXT NOT NULL,
  ...
  `);
  await pool.query('CREATE INDEX IF NOT EXISTS idx_gps_anomaly_company ON gps_anomaly_flags(company_id, created_at DESC);');

  // ---------------------------------------------------------------------------------------
  // ATTENDANCE REGULARIZATION — an employee who missed a punch (phone died, GPS failed,
  // forgot) can request a correction instead of having no way to report it at all.
  // ---------------------------------------------------------------------------------------
  await pool.query(`
    CREATE TABLE IF NOT EXISTS regularization_requests (
      ...
      requested_status TEXT NOT NULL, -- 'on_duty' | 'off_duty'
      requested_time TIME,
  ...
  `);
  await pool.query('CREATE INDEX IF NOT EXISTS idx_regularization_company ON regularization_requests(company_id, status, created_at DESC);');
  // Marks an attendance row as inserted via an approved regularization (rather than a
  // real punch), and links back to the request that created it — routes/regularization.js
  // writes both of these on approval.
  await pool.query('ALTER TABLE attendance ADD COLUMN IF NOT EXISTS is_regularized BOOLEAN NOT NULL DEFAULT FALSE;');
  await pool.query('ALTER TABLE attendance ADD COLUMN IF NOT EXISTS regularized_request_id INTEGER REFERENCES regularization_requests(id);');
  ...
  // ---------------------------------------------------------------------------------------
  // INCIDENT / DUTY REPORTS — separate from grievance.js (which is an employee's own
  // complaint). This is for reporting things *observed on duty*: a broken lock, a
  // suspicious person, a gate left open, etc. — with an optional photo.
  // ---------------------------------------------------------------------------------------
  await pool.query(`
  ...
  `);
  await pool.query('CREATE INDEX IF NOT EXISTS idx_incident_reports_company ON incident_reports(company_id, created_at DESC);');
  await pool.query('CREATE INDEX IF NOT EXISTS idx_incident_reports_employee ON incident_reports(employee_id, company_id);');

  // ---------------------------------------------------------------------------------------
  // SHIFT ROSTER — advance scheduling: which employee is assigned to which site on which
  // future date, with which shift. Distinct from the day-of "on_duty/off_duty" attendance
  // status — this is the PLAN, attendance is what actually happened.
  // ---------------------------------------------------------------------------------------
  await pool.query(`
    CREATE TABLE IF NOT EXISTS shift_roster (
      ...
      created_by TEXT,
      created_at TIMESTAMPTZ DEFAULT NOW(),
      UNIQUE(company_id, employee_id, roster_date)
    );
  `);
  await pool.query('CREATE INDEX IF NOT EXISTS idx_shift_roster_employee ON shift_roster(company_id, employee_id, roster_date);');
  await pool.query('CREATE INDEX IF NOT EXISTS idx_shift_roster_date ON shift_roster(company_id, roster_date);');
  // shift_start_time is stored as free text "HH:MM" against a specific roster_date, since
  // shift_categories.js already has named categories (Morning/Evening/Night) but no exact
  // clock time — the reminder job below needs a real time to count down from.
  ...
  `);
  await pool.query('CREATE INDEX IF NOT EXISTS idx_referrals_referrer ON employee_referrals(company_id, referrer_employee_id);');

  // ---------------------------------------------------------------------------------------
  // PUBLIC HOLIDAY CALENDAR — per-company. Kept intentionally simple (date + name); leave
  // balance calculations, etc. can reference this table later without a schema change.
  // ---------------------------------------------------------------------------------------
  await pool.query(`
  ...
  `);
  await pool.query('CREATE INDEX IF NOT EXISTS idx_company_holidays ON company_holidays(company_id, holiday_date);');

  // ---------------------------------------------------------------------------------------
  // MANAGER PERFORMANCE REVIEWS — qualitative complement to the auto-computed attendance-
  // based score in routes/performance.js. That score answers "did they show up, on time,
  // without incidents" — it can't see actual work quality, teamwork, or anything a manager
  // would judge by observation. One row per review (not one row per employee) so a full
  // rating history is kept rather than a single mutable "current rating" field.
  // ---------------------------------------------------------------------------------------
  await pool.query(`
  ...
  `);
  await pool.query('CREATE INDEX IF NOT EXISTS idx_performance_reviews_employee ON performance_reviews(company_id, employee_id, created_at DESC);');
