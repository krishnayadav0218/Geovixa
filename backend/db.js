...
async function init() {
  ...
  // Security fix: Employee ID alone used to be enough to log in and punch attendance for
  // someone else (an Employee ID is not a secret — it's printed on ID cards, seen on
  // reports, spoken out loud). pin_hash holds a bcrypt-hashed 4-6 digit PIN that the admin
  // sets per employee; login now requires Employee ID + PIN together.
  ...
  // One-time migration: any employee added before the PIN requirement existed has no
  // pin_hash yet. Auto-generate a random 4-digit PIN for each, hash it (never store plain-
  // text in the DB), and write the plaintext PINs to a local file ONCE so the admin can
  // securely distribute them to affected employees. Never overwrites an employee that
  // already has a pin_hash, and never re-runs for an employee once it's set.
  const legacyNoPin = await pool.query('SELECT employee_id FROM employees WHERE pin_hash IS NULL');
  if (legacyNoPin.rows.length > 0) {
    const bcrypt = require('bcryptjs');
    const { generateRandomPin } = require('./policy');
    const fs = require('fs');
    const path = require('path');
    const DATA_DIR = process.env.DATA_DIR || __dirname;
    const outFile = path.join(DATA_DIR, `generated_employee_pins_${Date.now()}.txt`);
    const lines = ['Employee ID, Generated PIN', '(Auto-created because these employees pre-date the PIN login requirement.)', '(Distribute securely to each employee, then delete this file.)', ''];
    for (const row of legacyNoPin.rows) {
      const pin = generateRandomPin();
      const hash = bcrypt.hashSync(pin, 10);
      await pool.query('UPDATE employees SET pin_hash = $1 WHERE employee_id = $2', [hash, row.employee_id]);
      lines.push(`${row.employee_id}, ${pin}`);
    }
    fs.writeFileSync(outFile, lines.join('\n'));
    console.log(`\n🔑 Generated login PINs for ${legacyNoPin.rows.length} existing employee(s) that didn't have one.`);
    console.log(`   Plaintext PINs written to: ${outFile}`);
    console.log('   Distribute these to employees securely, then delete the file.\n');
  }
