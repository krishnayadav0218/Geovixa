...
- **Nearby Search** — new panel in Reliever Management: pick a site, get every on-duty employee within a radius, sorted by live distance.
...
1. **Works today**: open the app in Chrome on Android → menu → "Add to Home Screen" — installs
   like a native app icon, works offline-tolerant for the UI shell.
2. **Real .apk**: wrap this same web app with Capacitor (`@capacitor/core` + `@capacitor/android`,
   both on npm) — ask for the ready-to-build Capacitor project scaffold and build instructions
   separately; building the final .apk itself requires Android Studio on your own machine.
